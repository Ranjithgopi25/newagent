...
def merge_two_editor_results(
    ...
    Strategy:
    - For suggested_text: Use result2's suggestions (applied on top of result1)
    - For feedback_edit: Combine feedback from both editors
    - Original text comes from result1 (first editor)
    
    Args:
        result1: First editor's result
        result2: Second editor's result (applied on top of result1)
        combined_editor_type: Combined editor type name (e.g., "development+content")
    
    Returns:
        Merged EditorResult with combined_editor_type
    """
    logger.info(f"MERGING {result1.editor_type} + {result2.editor_type} into {combined_editor_type}")
    
    # Create a map of blocks by ID for both results
    result1_blocks = {blk.id: blk for blk in result1.blocks}
    result2_blocks = {blk.id: blk for blk in result2.blocks}
    
    # Get all unique block IDs (union of both)
    ...
    for block_id in sorted(all_block_ids, key=lambda x: int(x[1:]) if x[1:].isdigit() else 999):
        ...
        # Determine original text (from first editor or second if first doesn't exist)
        original_text = blk1.original_text if blk1 else (blk2.original_text if blk2 else "")
        
        # Determine suggested text: prefer result2 (second editor), fallback to result1
        suggested_text = None
        if blk2 and blk2.suggested_text:
            suggested_text = blk2.suggested_text
        elif blk1 and blk1.suggested_text:
            suggested_text = blk1.suggested_text
        else:
        ...
        # Add feedback from first editor
        if blk1 and blk1.feedback_edit:
            combined_feedback.extend(blk1.feedback_edit)
        
        # Add feedback from second editor
        if blk2 and blk2.feedback_edit:
            combined_feedback.extend(blk2.feedback_edit)
        
        # Determine block metadata (type, level) - prefer first editor's metadata
        block_type = blk1.type if blk1 else (blk2.type if blk2 else "paragraph")
        block_level = blk1.level if blk1 else (blk2.level if blk2 else 0)
        
        # Determine has_changes
    ...
    # Combine warnings from both editors
    combined_warnings = list(result1.warnings) + list(result2.warnings)
...
def development_content_combined_node(state: SupervisorState) -> SupervisorState:
    ...
    # Track original results count
    original_count = len(state.get("editor_results", []))
    
    # Run article analysis if needed
    if not state.get("article_analysis"):
        state = {**state, **article_analysis_node(state)}
    
    # Run Development Editor (reuse existing node)
    ...
    # Run Content Editor on updated document (reuse existing node)
    content_state = content_editor_node({**dev_state, "document": updated_doc})
    content_result = content_state["editor_results"][-1]
    
    # Merge results - replace the two individual results with one merged result
    merged_result = merge_two_editor_results(dev_result, content_result, "development+content")
    
    return {
        "editor_results": state.get("editor_results", [])[:original_count] + [merged_result],
        "article_analysis": state.get("article_analysis"),
        "cross_paragraph_analysis": state.get("cross_paragraph_analysis")
    }
...
def line_copy_combined_node(state: SupervisorState) -> SupervisorState:
    ...
    # Run Line Editor (reuse existing node)
    ...
    # Run Copy Editor on updated document (reuse existing node)
