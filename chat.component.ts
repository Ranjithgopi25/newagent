...
{f"- CRITICAL: FINAL WORD COUNT LIMITS (NON-NEGOTIABLE)" if min_allowed_word_count is not None and max_allowed_word_count is not None else ""}
{f"- The FINAL word count MUST be between {min_allowed_word_count} and {max_allowed_word_count} words (inclusive)" if min_allowed_word_count is not None and max_allowed_word_count is not None else ""}
{f"- DO NOT exceed {max_allowed_word_count} words - this is a HARD MAX LIMIT" if max_allowed_word_count is not None else ""}
{f"- DO NOT go below {min_allowed_word_count} words - this is a HARD MIN LIMIT" if min_allowed_word_count is not None else ""}
- Acceptable range: {max(target_word_count - 5, min_allowed_word_count) if min_allowed_word_count is not None else target_word_count - 5} to {min(target_word_count + 5, max_allowed_word_count) if max_allowed_word_count is not None else target_word_count + 5} words
- Current: {current_word_count} words → Target: {target_word_count} words{f" (Must be within {min_allowed_word_count}-{max_allowed_word_count} words range)" if min_allowed_word_count is not None and max_allowed_word_count is not None else f" (Max allowed: {max_allowed_word_count} words - HARD LIMIT)" if max_allowed_word_count is not None else ""}
...
- Count your output words - if over target{f" or over {max_allowed_word_count}" if max_allowed_word_count is not None else ""}{f" or below {min_allowed_word_count}" if min_allowed_word_count is not None else ""}, adjust accordingly before finalizing
{f"- CRITICAL: Your output MUST be between {min_allowed_word_count} and {max_allowed_word_count} words - these are HARD LIMITS that cannot be violated." if min_allowed_word_count is not None and max_allowed_word_count is not None else f"- CRITICAL: Your output MUST NOT exceed {max_allowed_word_count} words - this is a HARD LIMIT that cannot be violated. Even if target is {target_word_count}, the maximum allowed is {max_allowed_word_count} words." if max_allowed_word_count is not None else ""}
