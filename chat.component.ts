...
# ------------------------------------------------------------
# MERGED EDITOR RESULTS VALIDATION PROMPT
# ------------------------------------------------------------

MERGED_EDITOR_VALIDATION_PROMPT = """
You are validating merged editor results at the PARAGRAPH BLOCK LEVEL. You must validate each paragraph block individually and provide strict, detailed feedback that combines rules, impact, and issues in a unified format.

============================================================
VALIDATION OBJECTIVE — STRICT REQUIREMENTS
============================================================

For EACH paragraph block that has been edited, you MUST provide validation feedback that includes:

1. RULE: The specific rule(s) that were applied or should have been applied
2. IMPACT: Why this change matters and what it achieves
3. ISSUE (Original): The exact problematic text from the ORIGINAL paragraph block
4. FIX (Final Edited Suggestion): The exact replacement text from the FINAL edited suggestion

============================================================
VALIDATION CRITERIA — PARAGRAPH BLOCK LEVEL
============================================================

For each paragraph block, validate:

1. ACCURACY OF CHANGES
   - Are the changes appropriate and necessary?
   - Do the changes align with the editor rules that were applied?
   - Are the changes consistent with the overall document objectives?

2. COMPLETENESS OF FEEDBACK
   - Does the feedback clearly identify the issue from the original text?
   - Does the feedback show the exact fix from the final edited suggestion?
   - Is the rule that was applied clearly stated?
   - Is the impact of the change explained?

3. QUALITY OF EDITS
   - Are the edits improving clarity, accuracy, or compliance?
   - Do the edits maintain the author's intent and meaning?
   - Are the edits consistent with the editor type(s) that were applied?

4. FEEDBACK FORMAT COMPLIANCE
   - Is the feedback structured correctly with Rule, Impact, Issue, and Fix?
   - Are the Issue and Fix texts exact quotes from the original and edited blocks?
   - Is the Rule clearly identified (e.g., "Development Editor - Central Argument Enforcement")?

============================================================
STRICT OUTPUT FORMAT — MANDATORY
============================================================

For EACH paragraph block that has changes, you MUST provide feedback in this EXACT format:

{
  ...
  "feedback_items": [
    {
      "rule_used": "[Editor Type] - [Specific Rule Name]",
      "impact": "Why this change matters and what it achieves",
      "issue": "EXACT problematic text from ORIGINAL paragraph block",
      "fix": "EXACT replacement text from FINAL edited suggestion",
      "priority": "Critical | Important | Enhancement"
    }
  ],
  "remarks": "Detailed explanation of validation findings for this block"
}

CRITICAL REQUIREMENTS:
- The "issue" field MUST contain the EXACT text from the original paragraph block
- The "fix" field MUST contain the EXACT text from the final edited suggestion
- The "rule_used" field MUST identify both the editor type and specific rule
- The "impact" field MUST explain why the change matters
- If multiple rules apply, create separate feedback items for each rule
...
MERGED EDITOR RESULT BLOCKS:
{merged_blocks}

EDITOR TYPES APPLIED: {editor_types}

============================================================
VALIDATION INSTRUCTIONS
============================================================

1. Compare each block in the merged result with its corresponding original block
2. For blocks with changes (has_changes = true):
   - Extract the exact original text (issue)
   - Extract the exact suggested text (fix)
   - Identify which editor rule(s) were applied
   - Explain the impact of the change
   - Validate that the change is appropriate and complete
3. For blocks without changes:
   - Validate that no changes were needed
   - If changes SHOULD have been made, flag as validation failure
4. Provide a score (0-10) for each block based on:
   - Accuracy of changes (0-3 points)
   - Completeness of feedback (0-3 points)
   - Quality of edits (0-2 points)
...
- 8-10: Block changes are accurate, complete, and properly documented
- 5-7: Block changes are partially correct but have notable gaps
- 0-4: Block changes are incorrect, incomplete, or improperly documented
...
You are consolidating editor feedback. When the same sentence has feedback from multiple editors, combine them:

1. If "issue" fields match (same sentence), merge into ONE feedback item:
   - Combine "rule_used": "[Editor1]+[Editor2] Editor - [Rule1] & [Rule2]"
   - Combine "impact": merge both impact statements into one comprehensive statement
   - Keep highest priority
   - Use the best "fix" (or combine if needed)

2. Keep non-duplicate feedback unchanged.

Return the same block structure with consolidated feedback.

{merged_result_json}

Return ONLY JSON array of blocks. No extra text.
"""
