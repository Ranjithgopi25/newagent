...
async def expand_grow_node(state: RefineGraphState, config: RunnableConfig):
    logger.info("[EXPAND_GROW] retry=%s | target=%s | current=%s | max_allowed=%s", 
                state.retry_count, 
                state.hard_target_word_count,
                state.current_word_count,
                state.max_allowed_word_count)
    ...
    # Early overshoot detection: check against max_allowed_word_count (includes 15% tolerance)
    if state.is_expand and state.max_allowed_word_count is not None and wc > state.max_allowed_word_count:
        overshoot = wc - state.max_allowed_word_count
        overshoot_pct = (overshoot / state.max_allowed_word_count * 100) if state.max_allowed_word_count > 0 else 0
        logger.warning(
            "[EXPAND_GROW] Overshoot detected: %s words (%.1f%% over max_allowed=%s) → TRIM",
            overshoot, overshoot_pct, state.max_allowed_word_count
        )
        return {"grown_content": grown, "current_word_count": wc, "next_step": "EXPAND_TRIM"}

    # Also check if significantly over hard target (even if within tolerance band)
    if (state.is_expand 
        and state.hard_target_word_count is not None 
        and wc > state.hard_target_word_count * 1.05):  # >5% over hard target
        logger.warning(
            "[EXPAND_GROW] Significant overshoot: %s words (target=%s) → TRIM",
            wc, state.hard_target_word_count
        )
        return {"grown_content": grown, "current_word_count": wc, "next_step": "EXPAND_TRIM"}

    logger.info("[EXPAND_GROW] Success: %s words (target=%s, max_allowed=%s)", 
                wc, state.hard_target_word_count, state.max_allowed_word_count)
...
async def expand_trim_node(state: RefineGraphState, config: RunnableConfig):
    ...
    hard_target = state.hard_target_word_count
    
    logger.info(
        "[EXPAND_TRIM] retry=%s | current=%s | hard_target=%s | max_allowed=%s",
        state.retry_count, latest_wc, hard_target, max_wc
    )

    # Prioritize hard_target_word_count as primary target for trimming
    # This ensures we aim for the actual target, not just within tolerance band
    if hard_target is not None:
        target_wc = hard_target
        logger.info("[EXPAND_TRIM] Using hard_target_word_count=%s as trim target", target_wc)
    elif state.ideal_word_count is not None:
        target_wc = state.ideal_word_count
        logger.info("[EXPAND_TRIM] Using ideal_word_count=%s as trim target", target_wc)
    elif state.min_allowed_word_count is not None:
        target_wc = state.min_allowed_word_count
        logger.info("[EXPAND_TRIM] Using min_allowed_word_count=%s as trim target", target_wc)
    else:
        # Fallback: aim for max_allowed if nothing else available
        target_wc = max_wc if max_wc is not None else latest_wc
        logger.warning("[EXPAND_TRIM] No target available, using fallback=%s", target_wc)
    ...
    prompt = build_compression_prompt(
        ...
        target_word_count=target_wc,
        current_word_count=current_wc,
        retry_count=state.retry_count,  # Now supports retry for trim operations
        previous_word_count=previous_wc,
    )
    ...
    reduction_achieved = current_wc - wc if current_wc else 0
    logger.info(
        "[EXPAND_TRIM] Result: %s words (reduced by %s, target was %s)",
        wc, reduction_achieved, target_wc
    )
...
def validate_node(state: RefineGraphState):
    ...
    if wc > max_wc and state.retry_count < state.max_retries:
        overshoot = wc - max_wc
        overshoot_pct = (overshoot / max_wc * 100) if max_wc > 0 else 0
        
        # For expansion overshoots, be more aggressive
        if state.is_expand:
            # Check if significantly over hard target (even if within tolerance band)
            if (state.hard_target_word_count is not None 
                and wc > state.hard_target_word_count * 1.05):  # >5% over hard target
                logger.warning(
                    "[VALIDATE] Expansion overshoot: %s words (%.1f%% over max_allowed=%s, target=%s) → AGGRESSIVE TRIM",
                    wc, overshoot_pct, max_wc, state.hard_target_word_count
                )
            else:
                logger.warning(
                    "[VALIDATE] Expansion overshoot: %s words (%.1f%% over max_allowed=%s) → TRIM",
                    wc, overshoot_pct, max_wc
                )
            return {
                "retry_count": state.retry_count + 1,
            ...
            logger.warning(
                "[VALIDATE] Compression undershoot: %s words (%.1f%% over max_allowed=%s) → COMPRESS",
                wc, overshoot_pct, max_wc
            )
            return {
                "retry_count": state.retry_count + 1,
