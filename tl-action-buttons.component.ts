...
class CommercialHubService:
    ...
    async def fetch_offerings_page(
        ...
        try:
            ...
            # Preferred shape: dict with "offerings" list, with fallbacks for other formats.
    ...
    async def fetch_offering_list(
        ...
        """
        Fetch offering list from the Commercial Hub offerings API.

        By default (pages is None), this will:
        - Start at page_number = 1
        - Fetch each subsequent page (2, 3, ...) in sequence
        - Stop when a page returns an empty offerings list

        If specific page numbers are provided in `pages`, only those pages
        will be requested.

        Returns list of {offering_id, page, offering_name, ...} from list API (no detail call).
        """
        async with httpx.AsyncClient() as client:
            ...
            # Default behaviour: paginate from page 1 until an empty offerings page is returned.
            current_page = 1
            while True:
                items = await self.fetch_offerings_page(client, current_page)
                if not items:
