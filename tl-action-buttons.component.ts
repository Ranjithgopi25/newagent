...
.contract-number-input:focus {
  outline: none;
  border-color: #fe7c39;
  box-shadow: 0 0 0 2px rgba(254, 124, 57, 0.15);
}
...
.editor-toggle-item {
  ...
  &:disabled + .editor-toggle-switch {
    background: #FFAA72;  // Exact same color as checked state
    color: white;
    cursor: not-allowed;
...
.final-output-content {
  ...
  display: block;
  font-size: 1.5em;          /* decreased from 2em */
  margin-top: 1em;
  margin-bottom: 0.5em;
  color: var(--text-primary);
  line-height: 1.2;
...
// Fixed spinner layer (stays on viewport)
