  ...
  private exportEditContentWord(): void {
    ...
    if (!blockTypes || !Array.isArray(blockTypes) || blockTypes.length === 0) {
      ...
      if (Array.isArray(paragraphEdits) && paragraphEdits.length > 0) {
        ...
        blockTypes = paragraphs.map((_: string, idx: number) => ({ index: idx, type: 'paragraph', level: 0 } as BlockTypeInfo));
      }
    }
    
    // Ensure content is plain text (strip HTML if present)
    ...
    const title = extractDocumentTitle(plainTextContent, this.metadata.topic);
    this.chatService.exportEditContentToWord({
      content: plainTextContent,  // Plain text content (not HTML formatted)
      title: title,
