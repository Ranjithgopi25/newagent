...
- NEVER add text outside the JSON response
        ...
        """
            ...
            if json_start != -1 and json_end > json_start:
                ...
                # Also check user input directly for editor names (more reliable than LLM)
                    ...
                    for editor in detected_editors_raw:
                        ...
                        for valid_id in valid_editor_ids:
                            if editor_lower == valid_id or editor_lower.replace(" ", "-") == valid_id:
                                # Double-check that this editor actually appears in user input
                                # Check for editor name patterns: "content editor", "content", "line editor", "line", etc.
                                editor_patterns = {
                                    ...
                                    "development": ["development editor", "development editing", "development edit", "development"],
                                    "brand-alignment": ["brand alignment", "brand-alignment", "brand editor", "brand"]
                                }
                                
                                patterns = editor_patterns.get(valid_id, [valid_id])
                                found_in_input = any(pattern in user_input_lower for pattern in patterns)
                                
                                if found_in_input and valid_id not in detected_editors:
                ...
                if "true" in response_lower or "yes" in response_lower or "edit" in response_lower:
                    return {
                        "is_edit_intent": True,
                        "confidence": 0.6,
