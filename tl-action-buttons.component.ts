...
WORKFLOW_SCHEMAS = {
    ...
    "enhance_with_research": {
        "required": ["original_content", "research_topics"],
        "optional": ["pwc_content", "proprietary", "thirdParty", "externalResearch"],
        "description": "Enhance content by adding research, data, and insights on specific topics"
    }
}
...
class TLAgent:
    ...
    async def _extract_parameters(self, state: AgentState) -> AgentState:
        ...
        if extracted_doc_content:
            ...
            if workflow_name == "format_translator":
                ...
                existing_outline = state["collected_params"].get("outline_doc", "")
                is_filename = (
                        existing_outline and 
                        len(existing_outline) < 200 and  # Filenames are short
                        any(existing_outline.endswith(ext) for ext in ['.docx', '.pdf', '.txt', '.doc'])
                    )
                if is_filename:
                    # Replace the filename with actual document content
                    state["collected_params"]["outline_doc"] = extracted_doc_content
                    logger.info(f"[DRAFT_CONTENT] Replaced outline_doc filename with actual content ({len(extracted_doc_content)} chars)")
                elif "supporting_doc" not in state["collected_params"]:
                    # Only use as supporting_doc if outline_doc already has real content
                    state["collected_params"]["supporting_doc"] = extracted_doc_content
                    logger.info(f"[DRAFT_CONTENT] Auto-extracted document as supporting_doc ({len(extracted_doc_content)} chars)")
        ...
        if workflow_name == "detect_edit_intent":
            ...
            """
        elif workflow_name == "enhance_with_research":
            extraction_prompt = f"""Extract parameters from the user's message for the enhance_with_research workflow.
    ...
    For enhance_with_research workflow:
    - original_content: The content to enhance with research
    - research_topics: Specific topics or questions to research (e.g., "how crypto will be shaped by nvidia")
    - pwc_content: Whether to include PWC content (optional, defaults to false)
    - proprietary: Proprietary sources configuration (optional)
    - thirdParty: Third-party sources configuration (optional)
    - externalResearch: Whether to use external research (optional, defaults to true)

    **RESEARCH TOPICS EXTRACTION:**
    - Look for phrases like "research on", "add research about", "enhance with data on"
    - Extract the specific topic/question after these phrases
    - If user says "add research" without specifying topic, DO NOT extract research_topics yet

    Return a JSON object with any NEW parameters found.
    Example: {{"research_topics": "impact of AI on healthcare industry"}}
    ...
    def _check_completeness(self, state: AgentState) -> AgentState:
        ...
        if workflow_name == "enhance_with_research":
            # Validate that research_topics is provided
            if "research_topics" not in collected or not collected.get("research_topics"):
                missing.add("research_topics")
    ...
    async def _ask_missing_params_stream(self, state: AgentState) -> AsyncGenerator[str, None]:
        ...
        if state.get("user_intent") == "unclear":
            if "refine" in user_message_text.lower():
                ...
                - Enhance with Research: Add research, data, and insights on specific topics
                ...
                Keep it conversational (2-3 sentences)."""
                    ...
                    if "original_content" in missing:
                        ask_prompt = """Generate a friendly message asking the user to provide the content they want to enhance with research.
            Instructions:
            - Keep it natural and conversational (2-3 sentences).
            - Example: "Could you paste the content you'd like me to enhance with research? I'll add relevant data, insights, and citations on the topics you specify."
            """
                    
                    elif "research_topics" in missing:
                        ask_prompt = """Generate a friendly message asking what specific topics they want research on.

            Instructions:
            - Use natural language
            - Ask for specific research topics or questions
            - Example: "What specific topics or questions would you like me to research? For example, 'impact of AI on healthcare' or 'crypto regulation trends'."
            ...
            formatted_workflow_name = workflow_name.replace('_', ' ').title() if workflow_name else ""
            ...
            if workflow_name == "draft_content":
                ...
                Workflow: {formatted_workflow_name}
    ...
    async def _execute_workflow(self, state: AgentState) -> AgentState:
        ...
        if workflow_name == "detect_edit_intent":
            ...
            result = await self._execute_enhance_with_research(params)
            state["execution_result"] = result
        else:
    ...
    async def _execute_enhance_with_research(self, params: dict) -> dict:
        """Execute enhance with research workflow"""
        ...
        try:
            logger.info(f"[EnhanceWithResearch] Starting content enhancement with research")
            ...
            if not original_content or not original_content.strip():
                return {
                    "status": "error",
                    "workflow": "enhance_with_research",
                    "error": "Original content is required"
                }
            
            if not research_topics or not research_topics.strip():
                return {
                    "status": "error",
                    "workflow": "enhance_with_research",
                    "error": "Research topics are required"
                }
            ...
            research_config = {
                ...
                "pwc_content": {
                    "isSelected": params.get('pwc_content', {}).get('isSelected', False)
                },
                "proprietary": {
                    "isSelected": params.get('proprietary', {}).get('isSelected', True),
                    "sources": params.get('proprietary', {}).get('sources', [
                        "pwc_industry_edge",
                        "pwc_com",
                        "pwc_insights",
                        "s_b_journal",
                ...
                "thirdParty": {
                    "isSelected": params.get('thirdParty', {}).get('isSelected', True),
                    "sources": params.get('thirdParty', {}).get('sources', [
                ...
                "externalResearch": {
                    "isSelected": params.get('externalResearch', {}).get('isSelected', True)
                }
            }
            
            # ✅ KEY CHANGE: Build services array matching the document structure
            services = [
                {
                    "isSelected": True,
                    "type": "expand",
                    "original_word_count": original_word_count,
                    "expected_word_count": original_word_count
                },
                {
                    "isSelected": False,
                    "type": "compress"
                ...
                research_config,  # ✅ This is the only selected service
                {
                    "isSelected": False,
                    "type": "edit_content",
                    "editors": []
            ...
            # ✅ KEY CHANGE: Add messages array (can be empty or minimal)
            request_data = {
                "messages": [
                    {
                        "role": "user",
                        "content": f"Please enhance the following content with research on: {research_topics}"
                    }
                ],
            ...
            logger.info(f'[EnhanceWithResearch] Data passed to refine_content_service: {request_data}')
            ...
            logger.error(f"Error executing enhance_with_research: {e}", exc_info=True)
            return {
                "status": "error",
                "workflow": "enhance_with_research",
                "error": str(e)
...
"""
            ...
            if json_start != -1 and json_end > json_start:
                ...
                # Validate and map detected editors to valid legacy IDs (brand/brand editor/brand edit -> brand-alignment)
                valid_editor_ids = ["line", "copy", "content", "brand-alignment"]
                brand_aliases = ("brand", "brand editor", "brand edit", "brand alignment")
                detected_editors_legacy = []
                
                if isinstance(detected_editors_raw, list):
                    for editor in detected_editors_raw:
                        editor_lower = str(editor).lower().strip()
                        if editor_lower in brand_aliases or editor_lower == "brand":
                        ...
                        for valid_id in valid_editor_ids:
                            if editor_lower == valid_id or editor_lower.replace(" ", "-") == valid_id:
                                if valid_id not in detected_editors_legacy:
                ...
                if "content" in detected_editors_legacy:
                    detected_editors.append("content")
                if any(e in detected_editors_legacy for e in ("line", "copy")):
                ...
                # If we detected editors from keywords (e.g. "strengthen my document" -> content), treat as edit intent so next step runs
                if detected_editors and not is_edit_intent:
                    is_edit_intent = True
                    logger.info(f"Intent: set is_edit_intent=true from keyword-detected editors: {detected_editors}")
