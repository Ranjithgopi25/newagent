...
- If the document already has citation numbers (e.g. Ref. 1, [1], ¹, (Ref. 2)), preserve and use those numbers consistently. Do not renumber. Convert only the format to superscript (¹ ² ³) and ensure the References list uses the same numbers (1., 2., 3.).
- If there are no citation numbers in the body but a References/Sources section exists, assign numbers 1., 2., 3. to the References list in order and add matching superscripts (¹ ² ³) in the body where each source is cited (e.g. at the end of the relevant sentence).
- If there are no numbers anywhere, generate them: number the References list 1. 2. 3. in order and use matching superscripts in the text for each citation.
...
EXCEPTION — CITATION SUPERSCRIPT LINKS (keep as-is for clickable superscript):
- Do NOT convert citation superscript links. Links whose text is only a superscript digit (e.g. [¹](https://...), [²](url)) must remain as markdown links so the superscript is clickable. Keep them exactly as [¹](URL), [²](URL), etc. If the document uses this format, leave it unchanged.
...
- Convert markdown links `[Title](URL)` to format: `Title [URL]` (except superscript-only links [¹](URL), [²](URL), etc. — keep those as-is)
...
- Citation numbers: existing numbers in the document are preserved and used consistently; if none existed, numbers were generated so References list is 1. 2. 3. and body uses matching superscripts ¹ ² ³.
...
- Inline citations: preserve existing format (superscript ¹²³⁴⁵⁶⁷⁸⁹ or [¹](URL) for clickable). Keep Title [URL] as-is. Do not remove or break links.
...
- Preserve inline citation format (superscript or [¹](URL)) exactly as in the input. Do not convert [¹](URL) or [²](URL) to any other format; leave as-is for clickable superscript.
