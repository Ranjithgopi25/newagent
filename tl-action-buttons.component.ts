...
# ----------------------------------------------------------------------
def run_editor_engine(editor_type: EditorName, blocks: list[DocumentBlock], article_analysis_text: Optional[str] = None, cross_paragraph_analysis_text: Optional[str] = None, validation_feedback: Optional[List[ValidationFeedbackItem]] = None) -> EditorResult:
...
"""
        ...
        if validation_feedback:
            ...
            if failed_criteria:
                ...
                feedback_context += "The previous attempt did not meet these criteria. You MUST address these issues:\n\n"
                for i, fb in enumerate(failed_criteria, 1):
                    feedback_context += f"{i}. FEEDBACK: {fb.feedback}\n"
                    feedback_context += f"   REMARKS: {fb.remarks}\n\n"
                feedback_context += "Focus on these specific issues to improve the validation score.\n\n"
