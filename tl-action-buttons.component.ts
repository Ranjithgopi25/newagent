...
💡 **Tips:** Type your request naturally, or click "Guided Journey" for a step-by-step wizard to create comprehensive content!`;
  ...
  onGenerateFinalArticle(message: Message): void {
    console.log('[ChatComponent] onGenerateFinalArticle called for message with paragraphEdits:', 
      message.editWorkflow?.paragraphEdits?.length || 0);
    
    if (message.editWorkflow?.paragraphEdits && message.editWorkflow.paragraphEdits.length > 0) {
      // Sync paragraph edits from message to service state BEFORE generating final article
      // This ensures we have the latest approval states from the UI
      console.log('[ChatComponent] Syncing paragraph edits to service state...');
      this.editWorkflowService.syncParagraphEditsFromMessage(message.editWorkflow.paragraphEdits);
    }
    ...
    // Generate final article using synced state
    console.log('[ChatComponent] Calling generateFinalArticle...');
  ...
  onNextEditor(message: Message): void {
    ...
    // Immediately update currentEditorIndex for visual feedback (like guided journey)
    // This makes the next editor show "In Progress" immediately with blinking animation
    if (message.editWorkflow && message.editWorkflow.currentEditorIndex !== undefined) {
      const nextIndex = message.editWorkflow.currentEditorIndex + 1;
      if (nextIndex < (message.editWorkflow.totalEditors || 0)) {
        message.editWorkflow.currentEditorIndex = nextIndex;
        // Update currentEditor if editorOrder is available
        if (message.editWorkflow.editorOrder && message.editWorkflow.editorOrder[nextIndex]) {
          message.editWorkflow.currentEditor = message.editWorkflow.editorOrder[nextIndex];
        }
        this.cdr.detectChanges();
      }
    }
...
Click one of the buttons below to get started, or you can type your selection.`,
  ...
  private getBlockTypesForExport(metadata: any, content: string): { content: string; blockTypes: BlockTypeInfo[] } {
    ...
    if (metadataBlockTypes && Array.isArray(metadataBlockTypes) && metadataBlockTypes.length > 0) {
      ...
      console.log('[Chat Component] Export block_types distribution:', typeCounts);
      console.log('[Chat Component] Total block_types:', blockTypes.length, 'Sample:', blockTypes.slice(0, 5));
    } else {
      ...
      console.warn('[Chat Component] Backend did not provide block_types, using default paragraph');
    ...
    return { content: finalContent, blockTypes };
  }
  ...
  private exportEditContentWord(messageIndex: number, message: Message, metadata: any): void {
    ...
    let exportBlockTypes: BlockTypeInfo[] | undefined = undefined;
    ...
    if (paragraphEdits && paragraphEdits.length > 0 && originalContent) {
      ...
      exportBlockTypes = blockTypes;  // blockTypes is BlockTypeInfo[]
      exportTitle = extractDocumentTitle(normalizedContent, metadata?.topic);
    }
    
    // Store exportTitle in a const for use in callback
    ...
    this.chatService.exportEditContentToWord({
      ...
      title: exportTitle,
      block_types: exportBlockTypes || undefined,  // Only used if paragraph_edits not available
