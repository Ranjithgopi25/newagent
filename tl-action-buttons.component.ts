...
Examples:
- "edit with line editor" → detected_editors: ["line"]
- "use copy editor" → detected_editors: ["copy"]
- "brand editor" → detected_editors: ["brand-alignment"]
- "content and development editor" → detected_editors: ["content", "development"]
...
- NEVER add text outside the JSON response
        ...
        """
            ...
            if json_start != -1 and json_end > json_start:
                ...
                # Mapping from user input variations to valid editor IDs
                editor_mapping = {
                    "line": "line",
                    "copy": "copy",
                ...
                if isinstance(detected_editors_raw, list):
                    for editor in detected_editors_raw:
                        ...
                        # Check mapping first
                        if editor_lower in editor_mapping:
                            mapped_id = editor_mapping[editor_lower]
                            if mapped_id not in detected_editors:
                                detected_editors.append(mapped_id)
                        else:
                            # Direct match or partial match for valid IDs
...
"""
    ...
    async def detect_intent(self, user_input: str) -> Dict[str, any]:
        """
        Combined intent detection that checks for both edit and draft intents.
        Returns an error message if no intent is detected.
        
        Args:
            user_input: User's input text to analyze
            
        Returns:
            dict with intent type, details, and error if no intent detected
        """
        try:
            # Check for edit intent first
            edit_result = await self.detect_edit_intent(user_input)
            
            if edit_result.get("is_edit_intent", False):
                logger.info(f"Edit intent detected: {edit_result}")
                return {
                    "intent_type": "edit",
                    "is_intent_detected": True,
                    "details": edit_result,
                    "error": None
                }
            
            # Check for draft intent if no edit intent
            draft_result = await self.detect_draft_intent(user_input)
            
            if draft_result.get("is_draft_intent", False):
                logger.info(f"Draft intent detected: {draft_result}")
                return {
                    "intent_type": "draft",
                    "is_intent_detected": True,
                    "details": draft_result,
                    "error": None
                }
            
            # No intent detected - return error message
            logger.warning(f"No intent detected for input: {user_input[:100]}...")
            return {
                "intent_type": None,
                "is_intent_detected": False,
                "details": None,
                "error": "Unable to understand your request. Please specify what you want to do:\n\n• To **edit content**, use words like 'edit', 'editing', 'editor' and specify the editor type (line, copy, brand, development, content)\n\n• To **create new content**, specify what you want to write (article, blog, white paper, executive brief) and include a topic\n\nExample: 'Edit my content with line editor' or 'Write an article about AI'"
            }
            
        except Exception as e:
            logger.error(f"Error in combined intent detection: {str(e)}", exc_info=True)
            return {
                "intent_type": None,
                "is_intent_detected": False,
                "details": None,
                "error": f"An error occurred while processing your request: {str(e)}. Please try again."
            }
