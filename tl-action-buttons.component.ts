...
def export_to_pdf_with_pwc_template(content: str, title: str = "Document", subtitle: str = "", content_type: Optional[str] = None) -> bytes:
    ...
    try:
        ...
        # Only extract ## markdown headers (main sections), ignore ### (subsections) and bold text
        headings = []
        ...
        for block in blocks:
            ...
            # Only extract ## markdown headings (main sections)
            # Ignore ### (subsections) and any bold text patterns
            if block.startswith('## ') and not block.startswith('###'):
                text = re.sub(r'^##\s*', '', block.split('\n')[0]).strip()
                text = text.replace('**', '').strip()
                # Remove leading numbers like "1. ", "5.1. ", etc.
                text = re.sub(r'^\d+(\.\d+)*\.?\s*', '', text).strip()
                if text.lower() in EXCLUDED_TOC_HEADINGS:
                    continue
        ...
        for block in blocks:
            ...
            block = block.strip()
            # Stop collecting headings once citations section starts
            clean_block = block.replace('*', '').strip().lower()
            if clean_block in EXCLUDED_TOC_HEADINGS:
                continue
            ...
            if in_citation_section:
                ...
                for line in lines:
                    ...
                    if clean_line.startswith('http'):
                        story.append(Paragraph(_format_content_for_pdf(clean_line), citation_style))
                        continue
                    # Fallback: capture any remaining text (like disclaimer)
            ...
            if block.startswith('####'):
                ...
                # Format and normalize text for PDF rendering
                text = _format_content_for_pdf(text)
                story.append(Paragraph(text, citation_subsection_style))
                continue
            elif block.startswith('##'):
                text = block.replace('##', '').strip()
                # Remove ** markers if present
        ...
        toc_pages = []
        headings = [h for h in headings if h.lower() not in EXCLUDED_TOC_HEADINGS]
        if headings and should_add_toc:
            ...
            for index, heading in enumerate(headings, start=1):
                ...
                formatted_heading = re.sub(r'^\d+(\.\d+)*\.?\s*', '', formatted_heading)
...
def _extract_headings_from_content(content: str) -> List[str]:
    ...
    Only extracts ## markdown headers (main sections), ignoring ### (subsections) and bold text.
    ...
    def is_excluded_heading(text: str) -> bool:
        ...
        return normalized in EXCLUDED_TOC_HEADINGS
    
    # Split content by lines to properly detect ## headers
    blocks = content.split('\n\n')
    ...
    for block in blocks:
        ...
        # Only extract ## markdown headings (main sections)
        # Ignore ### (subsections) and #### (sub-subsections) and any bold text patterns
        # if block.startswith('## ') and not block.startswith('###'):
        # if block.startswith('# ') and not block.startswith('##'):
        ...
        if not is_citation_subsection and not is_excluded_heading(text):
            headings.append(text)
    
    # Legacy handling for single # headers (kept for backward compatibility)
    # This handles rare cases where content uses single # for main headers
    for block in blocks:
        block = block.strip()
        if not block:
            continue
        ...
        # if block.startswith('# '):
            ...
            text = text.replace('**', '').strip()
            text = re.sub(r'^\d+(\.\d+)*\.?\s*', '', text).strip()
            if not is_excluded_heading(text):
                headings.append(text)
...
def export_to_word_with_metadata(content: str, title: str, subtitle: Optional[str] = None, 
    ...
    if has_template_structure:
        ...
        pf.left_indent = DocxPt(0)
        pf.right_indent = DocxPt(0)
        pf.first_line_indent = DocxPt(0)
        # doc.paragraphs[0].paragraph_format.left_indent = DocxPt(0)
        # doc.paragraphs[0].paragraph_format.right_indent = DocxPt(0)
        # doc.paragraphs[0].paragraph_format.first_line_indent = DocxPt(0)
...
def _clean_bullet_text(text: str) -> str:
    ...
    # 🔧 FIX: normalize broken links BEFORE formatting
    # text = re.sub(r'\s*\n\s*', ' ', text)
    text = re.sub(r'[ \t]+', ' ', text)
    # return text.strip()
    return text
...
def _format_content_for_pdf_mi(text: str) -> str:
    ...
    if not text:
        return text
    # 🔧 FIX: normalize broken markdown links (even if URL is split across lines)
    ...
    # Handle double bracket citations [[1]](url) with https protocol
    text = re.sub(
        r'\[\[(\d+)\]\]\(\s*(https?://[^\)]+?)\s*\)',
        lambda m: (
            f'<link href="{m.group(2)}">'
            f'<font color="blue"><super>[{m.group(1)}]</super></font>'
            f'</link>'
        ),
        text
    )
    
    # Handle double bracket citations [[1]](non-https-url) - internal refs, relative paths, etc.
    text = re.sub(
        # r'\[\[(\d+)\]\]\(\s*([^https:][^\)]*?)\s*\)',
        r'\[\[(\d+)\]\]\(\s*((?!https?://)[^\)]+)\s*\)',
        lambda m: (
    ...
    # Handle single bracket citation links [1](url) with https protocol
    text = re.sub(
        r'\[(\d+)\]\(\s*(https?://[^\)]+?)\s*\)',
        lambda m: (
            f'<link href="{m.group(2)}">'
    ...
    # Handle single bracket citation links [1](non-https-url) - internal refs, relative paths, etc.
    text = re.sub(
        r'\[(\d+)\]\(\s*([^https:][^\)]*?)\s*\)',
        lambda m: (
            f'<link href="{m.group(2)}">'
...
# )  
    ...
    # text = re.sub(r'(?<!\*)\*(.+?)\*(?!\*)', r'<i>\1</i>', text)
    text = re.sub(r'(?<!\*)\*(.+?)\*(?!\*)', r'<i>\1</i>', text)
    # Join broken URLs split across lines
    text = re.sub(
        r'(https?://[^\s<>"\]]+)\s*\n\s*([^\s<>"\]]+)',
        r'\1\2',
        text
    ...
    text = re.sub(
    # r'(?<!href=")(?<!">)(https?://[^\s<>"\]\){}]+)',
    # r'(?<!href=")(?<!<link href=")(https?://[^\s<>"\]\){}]+)',
    # r'(?<!href=")(?<!href=\')(?<!<link href=")(https?://[^\s<>"\]\){}]+)',
    # r'(?<!href=")(?<!href=\')(?<!<link href=")(?<!">)(https?://[^\s<>"\]\)\},\.]+)',
    # r'(?<!href=")(?<!href=\')(?<!<link href=")(?<!">)(https?://[^\s<>"\]\)\},]+)',
    r'(?<!href=")(?<!href=\')(?<!<link href=")(https?://[^\s<>"\]\)\},]+)',
    r'<link href="\1"><font color="blue">\1</font></link>',
...
def export_to_pdf_with_pwc_template_with_bullets(
    ...
    subtitle: str | None = None,
    content_type: str | None = None,
    include_toc: bool = True) -> bytes:
    
    logger.info("[Export] PDF-PWC-BULLETS endpoint hit")
    title = re.sub(r'^[#\s]*', '', re.sub(r'\*+', '', title)).strip()
    # Remove title from content so it does not repeat in body
    escaped_title = re.escape(title)
    include_toc = include_toc or (
    isinstance(content_type, str)
    and content_type.strip().lower() in ["article", "refine-content"]
)
    logger.info(f">>>PDF>>>> include_toc={include_toc}")
    content = re.sub(
    ...
    if include_toc:
        ...
        for block in blocks:
            ...
            if re.match(r'^\s*[-_]{3,}\s*$', block):
                continue
            if re.search(r'citations?\s*&?\s*references', block, re.I):
                logger.info("[TOC] Citations section detected. Stopping TOC extraction.")
                headings.append("Citations & References")
                break
...
# )
    ...
    for i, block in enumerate(blocks):
        ...
        if sentence_count > 20:    
            ...
            for split_para in split_paragraphs:
                if split_para:
                    ...
                    para = Paragraph(
                            ...
                            _format_content_for_pdf_mi(split_para),
                            body_style
                        )
            ...
            clean_block = re.sub(
                # r'<sup>\[\[(\d+)\]\]\((https?://[^)]+)\)</sup>',
                r'<sup>\[\[(\d+)\]\]\(\s*(https?://[^)]+?)\s*\)</sup>',
                r'<super><link href="\2"><font color="blue">[\1]</font></link></super>',
                clean_block
            )
            # clean_block = re.sub(r'(?<!\*)\*(?!\*)', '', clean_block)
            ...
            style_to_use = italic_style if is_last_block else body_style
            para = Paragraph(formatted, style_to_use)
    ...
    if include_toc:
        ...
        for index, heading in enumerate(filtered_headings, start=1):
            clean_heading = re.sub(r'^\s*\d+(\.\d+)*\.?\s*', '', heading)
            logger.info(f"[TOC] Adding heading {index}: {clean_heading}")
            # toc_story.append(Paragraph(f"{index}. {heading}", toc_heading_style))
            toc_story.append(
                Paragraph(f"{index}. {clean_heading}", toc_heading_style)
            )
        toc_doc.build(toc_story)
        toc_buffer.seek(0)
        toc_reader = PdfReader(toc_buffer)
...
def export_to_word_pwc_no_toc(
    ...
    logger.info(f">WORD>>> include_toc={include_toc}")
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        body_path = render_markdown_to_docx(markdown_text, tmpdir, False)
        ...
        if include_toc:
            ...
            remove_page_break_before_toc(doc)
            # Remove empty Pandoc paragraphs between cover and TOC
            # body = doc.element.body
            # toc_found_index = None            
            # Find TOC position first
            ...
            # if toc_found_index is not None and toc_found_index > 2:
            #     # Remove empty paragraphs/sections between cover and TOC
            #     # Keep ALL tables (they have template design/backgrounds)
            #     elements_to_remove = []
            #     for i in range(2, toc_found_index):
            #         el = body[i]
            #         tag = el.tag.split('}')[-1]
                    
            #         if tag == 'p':
            #             # Check if paragraph is empty or only has spacing
            #             text = ''.join(el.itertext()).strip()
            #             # Check for page breaks that create blank pages
            #             has_page_break = bool(el.xpath('.//w:br[@w:type="page"]'))
                        
            #             # Remove if: completely empty OR only has page break formatting
            #             if (not text and not el.xpath('.//*')) or has_page_break:
            #                 elements_to_remove.append(el)
            #         # NEVER remove tables - they contain template backgrounds
                
            #     # Remove collected elements
            #     for el in elements_to_remove:
            #         body.remove(el)
            
            # # Also remove any extra sections created between cover and TOC
            # # This prevents the blank page (page 2) from appearing
            # sections = list(doc.sections)
            # logger.info(f"Sections before cleanup: {len(sections)}")
            
            # # Keep only the first section (cover), remove any middle sections
            # # The TOC section will be added fresh
            # sections_to_remove = []
            # for i in range(1, len(sections)):
            #     sections_to_remove.append(sections[i])
            
            # for section in sections_to_remove:
            #     try:
            #         section._sectPr.getparent().remove(section._sectPr)
            #     except:
            #         pass
            
            # logger.info(f"Sections after cleanup: {len(doc.sections)}")
            ...
            # logger.info(f"Document sections before TOC insertion: {len(sections)}")
            # if len(sections) > 1:
            #     toc_section = sections[1]
            # # else use the toc_section already created
            ...
            # # add_page_break_after_toc(doc)
            # sections = doc.sections
            # logger.info(f"Document sections after TOC insertion: {len(sections)}")
            # if len(sections) <= 2:
            #     body_section = doc.add_section(WD_SECTION.NEW_PAGE)
            # else:
        ...
        # enable_update_fields_on_open(doc)
        # disable_external_links_prompt(doc)
...
def render_markdown_to_docx(markdown_text: str, tmpdir: str, include_toc: bool = True) -> str:
    ...
    pypandoc.convert_text(
        ...
        format="markdown+pipe_tables-tex_math_dollars-raw_html",
...
def disable_external_links_prompt(doc):
    settings = doc.settings.element
    do_not_update = OxmlElement('w:doNotUpdateFields')
    do_not_update.set(qn('w:val'), 'false')
    settings.append(do_not_update)
...
def add_page_break_after_toc(doc):
    logger.info("Adding page break after TOC")
    body = doc.element.body
    ...
    for i, el in enumerate(body):
        ...
        if tag == "sdt":  # TOC block
            if i + 1 < len(body):
                if not body[i + 1].xpath(".//w:br[@w:type='page']", namespaces=NS):
                    ...
                    body.insert(i + 1, p)
                    logger.info("Page break added after TOC")
...
def remove_page_break_before_toc(doc):
    logger.info("Removing page break before TOC if present")
    ...
    for i, el in enumerate(body):
        ...
        # Pandoc TOC container
        if tag == "sdt":
            if i > 0:
                prev = body[i - 1]

                # if the previous element is a completely empty paragraph, drop it
                if prev.tag.endswith('p'):
                    text = ''.join(prev.itertext()).strip()
                    if not text:
                        body.remove(prev)
                        logger.info("Removed empty paragraph before TOC")
                        return

                # look for page break inside paragraph
                for br in prev.iter():
                    if br.tag.endswith('br') and br.get(qn('w:type')) == 'page':
                        prev.getparent().remove(prev)
                        logger.info("Removed page break before TOC")
                        return
