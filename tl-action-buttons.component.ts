...
import { extractDocumentTitle, convertMarkdownToHtml } from '../../core/utils/edit-content.utils';
...
Click one of the buttons below to get started, or you can type your selection.`,
  ...
  private exportEditContentWord(messageIndex: number, message: Message, metadata: any): void {
    ...
    if (!metadata?.formattedContent) {
      alert('Formatted content is not available yet.');
      this.isExporting[messageIndex] = false;
      return;
    }
    ...
    // Use already-formatted HTML from generateFinalArticle (no duplicate formatting)
    const formattedContent = metadata.formattedContent;
    const title = extractDocumentTitle(content, metadata?.topic);
