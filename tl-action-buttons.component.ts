...
- Where both editors suggest a change for the same line/sentence in a block, KEEP ONLY the Content Editor suggestion. Omit the Development Editor suggestion for that line.
- Only include Development Editor suggestions for lines/sentences that the Content Editor did not change.
- Include ALL Content Editor suggestions.
...
- "feedback_edit": array of SingleEditorFeedback. Apply the rule above: for each line/sentence, if both editors have an item addressing it, include only the Content item; otherwise include the Development or Content item. Preserve the exact structure: [ { "editor": "development" | "content", "items": [ { "issue", "fix", "impact", "rule_used", "priority" } ] } ]

Two feedback items address the "same line/sentence" when their "issue" (quoted original text) refers to the same or overlapping part of the block (e.g. same phrase, same sentence, or one contains the other). When in doubt, treat as same line and keep Content only.
