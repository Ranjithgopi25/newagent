...
def _format_content_with_block_types_word(doc: Document, content: str, block_types: list[dict] | None = None):
    ...
    # Create block type map - use index from block_type if available, otherwise use position
    # This ensures we can handle cases where indices might not be sequential
    block_type_map = {}
    for i, bt in enumerate(block_types):
        idx = bt.get("index", i)
        block_type_map[idx] = bt
    ...
    for idx, block in enumerate(paragraphs):
        ...
        if not block_info:
            # Fallback: use paragraph type if block_type missing (instead of failing)
            logger.warning(
                f"[Export] Block type missing for paragraph index {idx}. "
                f"Using default 'paragraph' type. "
...
def export_to_word_edit_content(
    ...
    Note: Content may be received as HTML from frontend (via formatFinalArticleWithBlockTypes).
    For edit content only: convert HTML to plain text for consistent backend processing.
    """
    # EDIT CONTENT ONLY: Convert HTML to markdown-like text if present
    # Frontend sends HTML from formatFinalArticleWithBlockTypes()
    # However, frontend should now send plain text, but we handle HTML conversion as fallback
    if '<' in content and '>' in content:
        logger.warning("[Export] Content contains HTML tags, converting to markdown. Frontend should send plain text.")
        content = html_to_marked_text(content)
    
    # Add sanity check if block_types provided
    if block_types:
        paragraphs = split_blocks(content)
        logger.info(f"[Export] Word edit export: {len(block_types)} block_types, {len(paragraphs)} content blocks")
        
        # Validate block_types have correct structure
        for i, bt in enumerate(block_types):
            if not isinstance(bt, dict):
                logger.warning(f"[Export] Block type {i} is not a dict: {bt}")
                continue
            if "type" not in bt:
                logger.warning(f"[Export] Block type {i} missing 'type' field, defaulting to 'paragraph'")
                bt["type"] = "paragraph"
            if "index" not in bt:
                logger.warning(f"[Export] Block type {i} missing 'index' field, using position {i}")
                bt["index"] = i
        
        if len(block_types) < len(paragraphs):
            logger.error(
                f"[Export] Block type mismatch in Word edit export: {len(block_types)} block_types provided, "
                f"but content has {len(paragraphs)} blocks after splitting. "
                f"This may cause formatting issues. Attempting to continue with available block_types."
            )
            # Don't raise error, but log warning - we'll use available block_types and default to paragraph for missing ones
    ...
    for idx, block in enumerate(paragraphs):
        ...
        if not block:
            ...
            if not block_info:
                # Fallback: use paragraph type if block_type missing (instead of failing)
                logger.warning(
                    f"[Export] Block type missing for paragraph index {idx} in PDF export. "
                    f"Using default 'paragraph' type. "
                    f"Total blocks: {len(paragraphs)}, provided block_types: {len(block_types)}"
                )
                block_info = {"type": "paragraph", "level": 0, "index": idx}
...
def export_to_pdf_edit_content(
    ...
    if block_types:
        paragraphs = split_blocks(content)
        logger.info(f"[Export] PDF edit export: {len(block_types)} block_types, {len(paragraphs)} content blocks")
        ...
        if len(block_types) < len(paragraphs):
            logger.error(
                f"[Export] Block type mismatch in PDF edit export: {len(block_types)} block_types provided, "
                f"but content has {len(paragraphs)} blocks after splitting. "
                f"This may cause formatting issues. Attempting to continue with available block_types."
            )
            # Don't raise error, but log warning - we'll use available block_types and default to paragraph for missing ones
