              ...
              <div class="editor-timeline horizontal">
                @for (editor of selectedEditorsForTimeline; track editor.id; let i = $index; let last = $last) {
                  @if (editorProgressList[i]) {
                    ...
                    >
                      <div class="timeline-marker" [ngClass]="{ 'blink-marker': editorProgressList[i].status === 'processing' && isEditorLoading }">
                        @if (editorProgressList[i].status === 'completed') {
                          ✓
                        } @else {
                          <span [ngClass]="{ 'blink-number': editorProgressList[i].status === 'processing' && isEditorLoading }">{{ i + 1 }}</span>
                        }
                      </div>
                      <div class="timeline-editor-name">
                        {{ editor.name }}
                      </div>
                      <div class="timeline-status" [ngClass]="{ 'loading-status': editorProgressList[i].status === 'processing' && isEditorLoading }">
