...
CRITICAL: Superscripts MUST be clickable — use the exact format below so the link is preserved. Never output plain ¹ ² ³ without the link wrapper.

Use EXACT format: <sup>[ [ⁿ](URL) ]</sup> — n = Unicode superscript: ¹ ² ³ ⁴ ⁵ ⁶ ⁷ ⁸ ⁹ (NOT plain 1, 2, 3). The [ⁿ](URL) makes the superscript clickable.
...
NEVER remove the URL from any link — especially inline in paragraphs. Every link must end as Title [URL] with the URL preserved in square brackets.
...
- INLINE PARAGRAPH LINKS: Always convert to Title [URL]. NEVER drop the URL from links in the middle of sentences. Removing the URL from an inline link is NON-COMPLIANT.
...
- Inline in paragraph: `According to [PwC research](https://www.pwc.com/research), the findings show...` → `According to PwC research [https://www.pwc.com/research], the findings show...` (URL MUST remain; never output "According to PwC research, the findings show..." without [URL])
...
- `1. PwC Global CEO Survey` (URL removed — NON-COMPLIANT)
- `According to PwC research, the findings show...` (URL removed from inline paragraph — NON-COMPLIANT; you MUST keep the URL as "PwC research [https://...]")
...
- All reference markers are converted to clickable superscripts (<sup>[ [ⁿ](URL) ]</sup>) — superscripts must be clickable, not plain ¹²³
...
- No link URLs have been removed or converted to HTML — especially inline in paragraphs: every inline link must still show Title [URL]
- No link titles have been removed (leaving only `[URL]`)
- All URLs are preserved in square brackets `[URL]` format
- Links in citation sections, inline in paragraphs, and elsewhere are all converted to the required format; URLs in inline paragraphs must NOT be removed
