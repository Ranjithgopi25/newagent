...
export class SlideCreationPromptFlowComponent implements OnInit, OnDestroy {
  ...
  private handleDraftResponse(res: Record<string, unknown>): void {
    ...
    if (status === 'draft_generated') {
      ...
      this.extractedFieldsBaselineSnapshot = JSON.parse(
        JSON.stringify(this.extractedFieldsState)
  ...
  /** Close the SOW modal and open the main chat with the draft in the composer (user sends when ready). */
  generateSowInChat(): void {
    if (!this.draftMarkdown?.trim()) {
      return;
    }
    const body = this.draftMarkdown.trim();
    const prefill = `Here is my Statement of Work draft:\n\n${body}\n\n---\nYou can refine this below or ask follow-up questions.`;
    this.ddcFlowService.closeFlowAndPrefillComposer(prefill);
  }
  ...
  async applyExtractedEditsAndRegenerate(): Promise<void> {
    if (!this.editingExtractedFields) {
      return;
    }
