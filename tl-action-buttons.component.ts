...
CRITICAL — OUTPUT MARKDOWN ONLY (NO RAW <sup> IN ARTICLE OR FIX):
- In suggested_text and in every "fix" field, use MARKDOWN only. Do NOT output raw HTML such as <sup> in the article text.
- For inline paragraph citations WITH a URL: use markdown link format [¹](URL), [²](URL). Example: "[1]https://example.com" → "[¹](https://example.com)"; "Text [1]https://example.com more" → "Text [¹](https://example.com) more". The UI and export will render this as superscript and clickable and will preserve the URL.
- For inline paragraph citations WITHOUT a URL: use [¹], [²], etc. (brackets + Unicode superscript digit only).
- When converting citation markers, ONLY convert the marker itself; DO NOT remove or modify any text that follows, including URLs. NEVER remove URLs.
- References list: plain 1., 2., 3. only — no superscript. Do NOT use superscript in the References list.
...
- For in-body/inline paragraph citations with a URL: output markdown [¹](URL) only (no <sup>). The display layer will show it as superscript and clickable. References list: plain 1., 2., 3. only.
...
- Use superscript format ([¹], [²], or for citations with URL use markdown [¹](URL)) ONLY for citations that appear in body/paragraph text (inline citations).
...
CRITICAL — OUTPUT MARKDOWN ONLY; PRESERVE URL (INLINE PARAGRAPHS ONLY):
- In the formatted article text, use MARKDOWN only. Do NOT output raw HTML such as <sup> in the article. The UI and export will render markdown links as superscript and clickable.
- When a citation marker in body/paragraph text is followed by or associated with a URL, use markdown: [¹](URL), [²](URL). Examples:
...
- Place superscripts immediately after the referenced text (no space before superscript)
- For multiple references in a paragraph, combine superscripts or use comma-separated format for clarity
- For in-body/inline paragraph citations with a URL: output markdown [¹](URL) only. Do NOT use superscript in the References list.
...
- Citation numbers: existing numbers are preserved; References list is 1. 2. 3. (plain numbers only); inline paragraph citations use [¹] [²] [³] or markdown [¹](URL) when URL present (UI/export show as superscript and clickable).
- In inline paragraph text only: reference markers as superscripts in brackets; with URL use markdown [¹](URL) only. References list: plain 1., 2., 3. only — no superscript.
