...
class RefineGraphState(BaseModel):
    ...
    edit_after_compress: bool = False
...
def planner_node(state: RefineGraphState):
    ...
    return {
        ...
        "research_topics": (research_input or {}).get("research_topics"),
        "apply_edit": bool(services.get("edit")),
        "edit_after_compress": bool(services.get("edit_after_compress")),
...
# ============================================================
# OUTPUT CHECK AND FIX (after markdown — validate and fix structure)
# ============================================================

async def output_check_fix_node(state: RefineGraphState, config: RunnableConfig):
    """Check and fix markdown output (strip fences, fix structure) without changing content."""
    content = _latest_text(state)
    if not content or not content.strip():
        return {"final_output": content}

    response = await llm.ainvoke(build_output_check_fix_prompt(content))
    fixed = (response.content or "").strip()
    if fixed.startswith("```"):
        fixed = fixed.split("\n", 1)[-1].rsplit("```", 1)[0].strip()
    return {"final_output": fixed}
