...
Click one of the buttons below to get started, or you can type your selection.`,
   ...
   */
  /**
   * Use exact backend final_article and block_types - same as UI display
   * No processing needed: final_article is plain text with "\n\n" separators
   * block_types are already correctly aligned from backend final article generation
   */
  private getBlockTypesForExport(metadata: any, content: string): { content: string; blockTypes: BlockTypeInfo[] } {
    ...
    // Use exact backend block_types - no processing needed
    // Backend returns: block_types with sequential indices matching final_paragraphs
    const blockTypes = metadata?.block_types || [];
    
    // Simple mapping to ensure correct format (index, type, level)
    const normalizedBlockTypes: BlockTypeInfo[] = blockTypes.map((bt: any) => ({
      index: bt.index ?? 0,
      type: bt.type || 'paragraph',
      level: bt.level ?? 0
    }));
    
    return { content: finalContent, blockTypes: normalizedBlockTypes };
  }
