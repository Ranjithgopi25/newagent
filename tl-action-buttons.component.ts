...
def _format_content_with_block_types_word(doc: Document, content: str, block_types: list[dict] | None = None):
    ...
    CRITICAL: block_types MUST align 1:1 with split_blocks(content) output.
    If alignment is broken, this function will raise ValueError instead of silently corrupting.
    """
    if not block_types:
    ...
    if len(block_types) < len(paragraphs):
        raise ValueError(
            f"Block type mismatch: {len(block_types)} block_types provided, "
    ...
    # Split content into paragraphs (already done above)
    ...
    for idx, block in enumerate(paragraphs):
        ...
        if not block_info:
            ...
            raise ValueError(
                f"Block type missing for paragraph index {idx}. "
...
def export_to_word_edit_content(
    ...
    Note: Frontend now sends formatted HTML content (from formatFinalArticleWithBlockTypes).
    Parse HTML directly to preserve structure, or use block_types for plain text.
    """
    # Check if content is HTML (from formatFinalArticleWithBlockTypes)
    html_pattern = re.compile(r'<[a-z][a-z0-9]*[^>]*>', re.IGNORECASE)
    is_html = html_pattern.search(content)
    ...
    if is_html:
        ...
        # Extract headings from plain text using block_types
        paragraphs = split_blocks(content)
        block_type_map = {bt.get("index", i): bt for i, bt in enumerate(block_types)}
    ...
    # ---------- Content with Block Types or HTML ----------
    references_heading_added = False
    ...
    if "references" in content.lower() or "references:" in content.lower():
        ...
        if is_html:
            ...
            # Use block_types formatting for plain text
        ...
        if is_html:
            ...
            # Use block_types formatting for plain text (fallback)
