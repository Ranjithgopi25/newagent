...
def development_editor_retry_node(state: SupervisorState) -> SupervisorState:
    ...
    if validation_result:
        # Parse validation result from previous attempt
        ...
        if validation_score is not None:
            logger.info(f"Previous validation score: {validation_score}/10 (Target: >= 8)")
        
        if validation_feedback and isinstance(validation_feedback, list):
            # Ensure feedback items are properly structured
            valid_feedback = []
            for fb in validation_feedback:
                if hasattr(fb, 'passed') and hasattr(fb, 'feedback') and hasattr(fb, 'remarks'):
                    valid_feedback.append(fb)
                else:
                    logger.info(f"Skipping invalid feedback item: {fb}")
            
            if valid_feedback:
                failed_criteria = [fb for fb in valid_feedback if not fb.passed]
                passed_criteria = [fb for fb in valid_feedback if fb.passed]
                
                logger.info(f"Parsed validation feedback: {len(failed_criteria)} failed, {len(passed_criteria)} passed")
                
                # Log failed criteria that need to be addressed
                if failed_criteria:
                    logger.info("Issues to address in this retry:")
                    for i, fb in enumerate(failed_criteria, 1):
                        logger.info(f"  {i}. {fb.feedback}")
                        if fb.remarks:
                            logger.info(f"     Details: {fb.remarks[:100]}...")  # Truncate for logging
                
                # Log passed criteria that should be maintained
                if passed_criteria:
                    logger.info(f"Maintaining {len(passed_criteria)} passed criteria")
                
                # Use validated feedback
                validation_feedback = valid_feedback
            else:
                logger.info("No valid validation feedback items found")
                validation_feedback = None
        else:
            logger.info("No validation feedback available from previous attempt")
    else:
        logger.info("No previous validation result found")
...
def article_validation_node(state: SupervisorState) -> SupervisorState:
    ...
    # Parse and log validation feedback
    if hasattr(validation_result, 'feedback_remarks') and validation_result.feedback_remarks:
        failed_criteria = [fb for fb in validation_result.feedback_remarks if not fb.passed]
        passed_criteria = [fb for fb in validation_result.feedback_remarks if fb.passed]
        
        logger.info(f"Validation results: {len(passed_criteria)} passed, {len(failed_criteria)} failed")
        
        if failed_criteria:
            logger.info("Failed criteria that need to be addressed:")
            for i, fb in enumerate(failed_criteria, 1):
                logger.info(f"  {i}. {fb.feedback}")
...
def cross_paragraph_validation_node(state: SupervisorState) -> SupervisorState:
    ...
    if not cross_paragraph_analysis_text or not cross_paragraph_analysis_text.strip():
        logger.info("No cross-paragraph analysis text found for validation")
        return {}
    
    editor_results = state.get("editor_results", [])
    if not editor_results:
        logger.info("No editor results found for validation")
        return {}
    ...
    if not content_editor_result:
        logger.info("Content Editor result not found for validation")
        return {}
    ...
    if warnings:
        ...
        logger.info(f"Cross-paragraph validation found {len(warnings)} issues: {warnings}")
    else:
        logger.info("Cross-paragraph validation: All requirements met")
