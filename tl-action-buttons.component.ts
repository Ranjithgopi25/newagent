          ...
          @if (sowDraftVersions.length > 0) {
            <div class="draft-result">
              @if (sowDraftVersions.length > 1) {
                <div class="sow-version-selector">
                  <label for="sow-version-select">Draft version</label>
                  <select
                    id="sow-version-select"
                    [ngModel]="selectedSowVersionIndex"
                    (ngModelChange)="selectSowVersion($event)">
                    @for (_v of sowDraftVersions; track $index) {
                      <option [ngValue]="$index">Version {{ $index + 1 }}</option>
                    }
                  </select>
                </div>
              }
              ...
              @if (showDraftResultHintAfterApplyRegenerate) {
                <p class="draft-result-hint">
                  After you regenerate, earlier drafts stay as Version 1, 2, … — use the dropdown to switch. Edits use the
                  latest version only. <strong>Generate SOW in chat</strong> sends whichever version is open.
                </p>
              }
