...
def build_risk_words_instruction(max_items: int = 80) -> str:
    ...
    lines: List[str] = [
        ...
        (
            "Scan the document for exact matches against the entries below (see "
            "RISK DETECTION PHASE and MATCHING RULES in this prompt). "
            "When a match requires remediation, prefer the suggested alternative "
            "where it does not change meaning or introduce guarantees."
        ),
        (
            "Do NOT treat every match as an automatic violation: apply EXPLICIT "
            "EXCEPTIONS and CONTEXT-DEPENDENT TERMS (optimize/maximize/minimize "
            "families) in this prompt before emitting Issue/Fix."
        ),
        "",
