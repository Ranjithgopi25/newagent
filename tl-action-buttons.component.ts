...
class CommercialHubService:
    ...
    async def fetch_offering_list(
        ...
        If `pages` is provided, fetch exactly those page numbers in parallel.
        If `pages` is None, fetch pages sequentially starting from 1 until an
        empty page is returned.

        Returns list of {offering_id, page, offering_name} from the list API
        (no detail calls).
        """
        ...
        async with httpx.AsyncClient() as client:
            if pages is not None:
                # Explicit page list: fetch in parallel
        ...
        logger.info("Fetched %d offerings across pages %s", id_total, fetched_pages)
