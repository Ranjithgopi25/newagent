...
# ----------------------------------------------------------------------
# SANITIZATION — strip markup that weak LLMs sometimes emit
# ----------------------------------------------------------------------
def _strip_markup_from_text(text: str) -> str:
    """
    Remove HTML/XML-like markup from suggested_text, issue, fix.
    Weak or prod-variant LLMs sometimes output literal span/class tags.
    """
    if not text or not isinstance(text, str):
        return text
    # Full tags: <span ...>, </span>
    text = re.sub(r"</?span[^>]*>", "", text, flags=re.IGNORECASE)
    # Malformed: span class="..." without leading <
    text = re.sub(r'span\s+class="[^"]*"\s*>', "", text, flags=re.IGNORECASE)
    # Other common markup
...
"""
    ...
    for block in doc.blocks:
        ...
        suggested = _strip_markup_from_text(suggested) if suggested else block.text
