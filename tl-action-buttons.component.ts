...
export class ChatComponent implements OnInit, OnDestroy, AfterViewChecked {
  ...
  // Request history - track current request source for banner title
  ...
  get accessibleModuleCount(): number {
    ...
    if (this.cortexAccessible) count++;
    return count;
  }
  ...
  ngOnInit(): void {
    ...
    if (userInfo && userInfo.email) {
      ...
      console.log('[ChatComponent] ✅ User logged in:', this.userId);
      
      // STEP 1: Apply module access from profile (already fetched and set in app.component)
      console.log('[ChatComponent] 📋 Applying module access from user profile...');
      const moduleAccess = this.authService.getModuleAccessForDisplay();
      
      this.docStudioAccessible = moduleAccess.docStudio;
      this.marketIntelligenceAccessible = moduleAccess.marketIntelligence;
      this.cortexAccessible = moduleAccess.cortex;
      
      // STEP 2: Update offering visibility to hide inaccessible modules
      this.offeringVisibility['ppt'] = this.docStudioAccessible;
      this.offeringVisibility['thought-leadership'] = this.cortexAccessible;
      this.offeringVisibility['market-intelligence'] = this.marketIntelligenceAccessible;
      
      console.log('[ChatComponent] ✅ Module visibility applied:', {
        ppt: this.docStudioAccessible,
        'thought-leadership': this.cortexAccessible,
        'market-intelligence': this.marketIntelligenceAccessible
      });
      
      // STEP 3: Load user profile and preload cache in background (non-blocking)
      this.loadUserProfile(userInfo.email);
      this.chatService.preloadChatHistoryCache(this.userId).subscribe({
        next: () => console.log('[ChatComponent] ✅ Cache preload successful'),
        error: () => console.warn('[ChatComponent] ⚠️ Cache preload failed (will use DB fallback)')
      });
      
    } else {
      this.userId = 'anonymous@example.com';
      console.log('[ChatComponent] ℹ️ No user logged in, using anonymous mode');
    ...
    this.subscribeToDdcComposerPrefill();
    ...
    try {
      ...
      console.log('[ChatComponent] 📱 Sidebar state:', { expanded: this.sidebarExpanded, mobile: this.mobileMenuOpen });
    } catch (e) {
      // ignore in non-browser environments
    }

    // Focus quick start button
    setTimeout(() => {
  ...
  private loadUserProfile(email: string): void {
    console.log('[ChatComponent] 🔄 Loading user profile (using cached version to avoid redundant API calls)...');
    
    // Use authService.getUserProfile() which has caching built-in
    this.authService.getUserProfile().then((response: any) => {
  ...
  /** When a DDC flow (e.g. SOW) closes and passes draft text, fill the composer and focus it. */
  private subscribeToDdcComposerPrefill(): void {
    this.ddcFlowService.composerPrefill$
      .pipe(
      ...
      .subscribe(text => {
        this.userInput = text;
        this.ddcFlowService.clearComposerPrefill();
        this.showLandingPage = false;
        this.cdr.detectChanges();
        setTimeout(() => {
  ...
  private async proceedWithNormalChat(trimmedInput: string): Promise<void> {
      ...
      this.chatService.ddcChatAgent(
        ...
        next: (result: any) => {
          ...
          if (result && result.blob) {
            ...
            // Use filename from response headers if available, otherwise generate fallback
            let filename: string;
            if (result.filename) {
              console.log('[ChatComponent] Using filename from Content-Disposition header:', result.filename);
              // Only add _sanitized suffix for sanitization workflows, not for create-PPT
              if (isSanitizationRequest) {
                // Ensure the filename has _sanitized suffix if it doesn't already
                if (!result.filename.includes('_sanitized')) {
                  const baseName = result.filename.replace(/\.[^/.]+$/, '');
                  const sanitized = this.sanitizeFilename(baseName);
                  filename = `${sanitized}_sanitized.pptx`;
                } else {
                  filename = result.filename;
                }
              } else {
                // For create-PPT, use the filename as-is without adding _sanitized
                filename = result.filename;
              }
            } else {
              filename = isSanitizationRequest ? this.getPptxDownloadFilename(pptFile, url, isSanitizationRequest) : 'Generated_Presentation.pptx';
            }
            ...
            if (urlMatch && urlMatch[1]) {
              ...
              assistantMessage.downloadFilename = isSanitizationRequest ? this.getPptxDownloadFilename(pptFile, downloadUrl, isSanitizationRequest) : 'Generated_Presentation.pptx';
              downloadGenerated = true; // Mark that download was generated
              // Reset conversation_id for next message to start fresh conversation
              this.currentDdcConversationId = null;
    ...
    // Track if we received actual content and metadata signals (same as TL Quick Request)
    ...
    this.chatService.streamChat(
      ...
      next: (content: any) => {
        console.log('[ChatComponent-MI] Received response - RAW:', content);
        console.log('[ChatComponent-MI] Type:', typeof content);
        console.log('[ChatComponent-MI] Is Object?:', typeof content === 'object' && content !== null);
        if (typeof content === 'object' && content !== null) {
          console.log('[ChatComponent-MI] Object keys:', Object.keys(content));
          console.log('[ChatComponent-MI] content.type:', content.type);
        }
        
        // First, check if this is a metadata or control signal (before any parsing)
        if (typeof content === 'object' && content !== null) {
          if (content.type === 'metadata') {
            console.log('[ChatComponent-MI] ❌ CAUGHT: Metadata signal detected - BLOCKING disclaimer:', content);
            receivedMetadata = true;
            return;
          }
          if (content.type === 'done' || content.done === true) {
            console.log('[ChatComponent-MI] Done signal received');
            return;
          }
        }
        
        // Handle case where metadata/signals come as JSON string
        if (typeof content === 'string') {
          try {
            const parsed = JSON.parse(content);
            console.log('[ChatComponent-MI] Parsed JSON string:', parsed);
            if (parsed && typeof parsed === 'object') {
              if (parsed.type === 'metadata') {
                console.log('[ChatComponent-MI] ❌ CAUGHT: Metadata JSON string detected - BLOCKING disclaimer:', parsed);
                receivedMetadata = true;
                return;
              }
            ...
            // Not JSON, treat as regular content string
          }
        }
        
        // Accumulate string content (only regular content, not signals)
        if (typeof content === 'string') {
          receivedActualContent = true;
          assistantMessage.content += content;
          console.log('[ChatComponent-MI] ✅ Accumulated content:', content.substring(0, 50));
          this.triggerScrollToBottom();
        }
      },
      error: (error: any) => {
      ...
      complete: () => {
        ...
        console.log('[ChatComponent-MI] ⚙️ Complete handler - Flag values:', {
          receivedActualContent,
          receivedMetadata,
          contentLength: assistantMessage.content?.length || 0,
          content: assistantMessage.content?.substring(0, 80) || '(empty)'
        });
        
        // Append disclaimer to MI-generated content only if:
        ...
        // 3. Content exists and is not an error message
        const isFallbackMessage = assistantMessage.content && 
          assistantMessage.content.includes('I am ready to help. What would you like to do?');
        ...
        if (receivedActualContent && 
          ...
          console.log('[ChatComponent-MI] ✅ Disclaimer appended to MI Quick Request response');
        } else if (!receivedActualContent) {
          console.log('[ChatComponent-MI] ❌ No actual content received - skipping disclaimer');
        } else if (receivedMetadata) {
          console.log('[ChatComponent-MI] ❌ Metadata detected in stream - skipping disclaimer (workflow interaction)');
        } else if (isFallbackMessage) {
          console.log('[ChatComponent-MI] ❌ Fallback message detected - skipping disclaimer');
        } else {
          console.log('[ChatComponent-MI] ❌ Other condition blocked disclaimer');
        }
      ...
      const subscription = this.chatService.streamTlChatAgent(
        ...
        next: (response: any) => {
          ...
          if (response && typeof response === 'object' && response.isStreamChunk === false) {
            ...
            // ===== CASE 0: CAPTURE X-CONTENT-TYPE HEADER FOR QUICK REQUEST EXPORT =====
            if (response.type === 'x-content-type' && response.contentType) {
              console.log('[ChatComponent] Captured x-content-type header:', response.contentType);
              // Store the content type header on the assistant message for later use during export
              (assistantMessage as any).headerContentType = response.contentType;
              // Don't return here - continue processing other signals in the stream
              return;
            }
  ...
  processSanitizePPT(): void {
    ...
    this.performSanitization(pptFile, '', '', null, assistantMessage);
  }

  /**
   * Unified method to perform PPT sanitization with consistent response handling
   */
  private performSanitization(pptFile: File, clientName: string, products: string, options: any, assistantMessage: Message): void {
    this.currentAction = 'Sanitizing presentation: removing sensitive data, client names, numbers, and metadata...';
    this.isLoading = true;

    this.chatService.sanitizePPT(pptFile, clientName, products, options).subscribe({
      next: (response) => {
        ...
        assistantMessage.downloadUrl = url;
        assistantMessage.downloadFilename = this.getSanitizedFilename(pptFile.name);
  ...
  selectFlow(flow: 'ppt' | 'thought-leadership' | 'market-intelligence'): void {
    ...
    // Clear request history source when switching flows so banner shows feature name
  ...
  goHome(): void {
    ...
    // Clear request history source when going home
    this.currentRequestSource = '';
  ...
  openUserGuide(): void {
    // Open user guide in new tab with security safeguards
    const guideUrl = 'https://pwc.sharepoint.com/:p:/s/US-IFS-ThinkSpace/IQCaY8IVgZYUT6c1qx-kEh7AAZMQsCiKAIPSDDUvM-O-Dh8?wdOrigin=TEAMS-MAGLEV.p2p_ns.rwc&wdExp=TEAMS-TREATMENT&wdhostclicktime=1775193019817&web=1&isSPOFile=1&ovuser=513294a0-3e20-41b2-a970-6d30bf1546fa%2Csoumyajyoti.mondal%40pwc.com&clickparams=eyJBcHBOYW1lIjoiVGVhbXMtRGVza3RvcCIsIkFwcFZlcnNpb24iOiI0OS8yNjAyMDEwMTEyMCIsIkhhc0ZlZGVyYXRlZFVzZXIiOmZhbHNlfQ%3D%3D';
    
    // Use window.open with noopener and noreferrer to prevent tabnabbing attacks
    const newWindow = window.open(guideUrl, '_blank', 'noopener,noreferrer');
  ...
  startNewChat(): void {
    ...
    // Clear request history source when starting new chat
    this.currentRequestSource = '';
  ...
  getFeatureName(): string {
    if (this.selectedFlow === 'ppt') {
      ...
      return 'Redline a contract';
    }
    return 'Contracting Hub';
    ...
    this.chatService.getSessionConversation(sessionId)
      ...
      .subscribe({
        next: (sessionDetail: any) => {
          ...
          // Extract source from the session detail response
          if (sessionDetail.source) {
            console.log('[ChatComponent] Extracting source from session detail response:', sessionDetail.source);
            this.setRequestHistorySource(sessionDetail.source);
          }
  ...
  sanitizePPT(): void {
    ...
    const pptFile = this.sanitizePPTFile;
    this.sanitizePPTFile = null;

    this.performSanitization(pptFile, this.sanitizeData.clientName, this.sanitizeData.products, this.sanitizeData.options, assistantMessage);

    // Reset sanitize data after processing
    this.sanitizeData = {
      clientName: '',
      products: '',
      options: {
 ...
 **Tips:** Type your request naturally, or click "Guided journey" for a step-by-step wizard to create comprehensive content!`;
  ...
  triggerReferenceUpload(): void {
    const fileInput = document.createElement('input');
    fileInput.type = 'file';
    fileInput.accept = '.pptx,.docx,.pdf,.txt,.xlsx';
    fileInput.onchange = (event: any) => {
      const file = event.target.files[0];
      if (!file) return;
...
private sanitizedFilename(filename: string): string {
  ...
  // Remove path traversal + dangerous chars
...
private getSafeMimeType(blob: Blob, ext: string): string {
  ...
  return allowedMimeTypes[ext] || 'application/octet-stream';
}
 
private createTrustedBlob(blob: Blob, ext: string): Blob {
  const mimeType = this.getSafeMimeType(blob, ext);
 
  // Explicitly block dangerous HTML/script MIME types
  const blockedTypes = [
    'text/html',
  ...
  const finalMimeType = blockedTypes.includes(mimeType)
    ? 'application/octet-stream'
    : mimeType;
 
  return new Blob([blob], { type: finalMimeType });
}
 
private isTrustedBlobUrl(url: string): boolean {
  try {
...
private async downloadBlobWithSaveDialog(
  blob: Blob,
  filename: string
): Promise<void> {
  const safeFilename = this.sanitizedFilename(filename);
  const ext = safeFilename.split('.').pop()?.toLowerCase() || 'bin';
  const safeBlob = this.createTrustedBlob(blob, ext);
  const mimeType = safeBlob.type;
  ...
  if ('showSaveFilePicker' in window) {
    try {
      ...
      // // Write ONLY trusted normalized blob
      // await writable.write(safeBlob);
      // await writable.close();
...
// Write raw binary instead of blob
...
private fallbackDownload(blob: Blob, filename: string): void {
  ...
  // Strong blob URL allowlist validation
  if (!this.isTrustedBlobUrl(url)) {
    URL.revokeObjectURL(url);
    throw new Error('Unsafe object URL blocked');
  }
...
private async exportPPT(
  ...
  try {
    ...
    console.error('[ChatComponent] API URL validation failed');
    this.toastService.error(
      'Invalid API endpoint. Request rejected for security reasons.'
    );
    this.isExporting[messageIndex] = false;
    return;
  }
  ...
  try {
    ...
    try {
      ...
      console.error(
        '[ChatComponent] PPT download URL validation failed'
      );
      this.toastService.error(
        'Invalid download URL. Download rejected for security reasons.'
      );
      this.isExporting[messageIndex] = false;
      return;
    }
 
    // ✅ SAFE URL DOWNLOAD (replaces fetch)
...
await writable.close();
  ...
  private async exportDocument(messageIndex: number, message: Message, metadata: any, endpoint: string, extension: string): Promise<void> {
    ...
    try {
      // Prioritize header content type from x-content-type header (Quick Request export)
      // Otherwise fall back to metadata content type
      const headerContentType = (this.messages[messageIndex] as any)?.headerContentType;
      const contentTypeToExport = headerContentType || metadata?.contentType;
      
      console.log('[ChatComponent] Exporting with content_type:', {
        source: headerContentType ? 'x-content-type header' : 'metadata',
        value: contentTypeToExport
      });
      ...
      const response = await this.authFetchService.authenticatedFetch(fullEndpoint, {
        ...
        body: JSON.stringify({
          ...
          title,
          subtitle: '',
          content_type: contentTypeToExport
        })
      });
  ...
  private getSanitizedFilename(originalFilename: string): string {
    // Remove file extension and sanitize the base name
    const baseName = originalFilename.replace(/\.[^/.]+$/, '');
    const sanitizedBase = this.sanitizeFilename(baseName);
    return `${sanitizedBase}_sanitized.pptx`;
  }

  private getPptxDownloadFilename(pptFile: File | undefined, downloadUrl?: string, isSanitization: boolean = false): string {
    if (pptFile) {
      if (isSanitization) {
        return this.getSanitizedFilename(pptFile.name);
      }
      return pptFile.name;
    }

    if (downloadUrl) {
      // Extract the last segment from URL (after final /)
      const lastSegment = downloadUrl.split('/').pop();
      
      // Remove query params/hash and check if it's a pptx file
      const filenameMatch = lastSegment?.match(/^([^?#]+\.pptx)/i);
      ...
      if (filenameMatch && filenameMatch[1]) {
        ...
        const sanitized = this.sanitizeFilename(baseName);
        if (isSanitization) {
          return `${sanitized}_sanitized.pptx`;
        }
        return `${sanitized}.pptx`;
      }
    }

    return isSanitization ? 'presentation_sanitized.pptx' : 'presentation.pptx';
  }
  ...
  onMyRequestsClick(): void {
    ...
    // Clear request history source when opening My Requests panel
    this.currentRequestSource = '';
  ...
  /**
   * Update banner title based on request history source
   * Call this method when a request history item is opened in the chat
   * 
   * @param source - The source field from the request history response JSON
   */
  setRequestHistorySource(source: string): void {
    if (source) {
      this.currentRequestSource = source;
      // Update selectedFlow to activate the corresponding icon-nav-btn
      const flowMap: { [key: string]: 'ppt' | 'thought-leadership' | 'market-intelligence' } = {
        'Doc_Studio': 'ppt',
        'Market_Intelligence': 'market-intelligence',
        'Cortex': 'thought-leadership',
      };
      if (flowMap[source]) {
        this.selectedFlow = flowMap[source];
      }
      // Close the My Requests panel to show the chat with updated banner
      this.showMyRequestsPanelParent = false;
      console.log('[ChatComponent] Request history source set:', { source });
      this.cdr.markForCheck();
    }
  }

  /**
   * Get the display name for a source value to show in the banner
   */
  getSourceDisplayName(source: string): string {
    const sourceMap: { [key: string]: string } = {
      'Doc_Studio': 'Doc studio',
      'Market_Intelligence': 'Market intelligence and insights',
      'Cortex': 'Cortex content studio',
    };
