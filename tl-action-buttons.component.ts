...
def _format_content_for_pdf(text: str) -> str:
    ...
    # Convert ==highlight== to <font backColor="yellow">highlight</font>
...
def _add_markdown_text_runs(paragraph, text: str,allow_bold: bool = True):
    ...
    *italic* text
    ...
    while pos < len(text_with_placeholders):
        ...
        # Check for highlight
        highlight_match = re.search(r'==(.+?)==', text_with_placeholders[pos:])
        # Check for italic
        ...
        if highlight_match:
            matches.append(('highlight', pos + highlight_match.start(), highlight_match))
        if italic_match:
        ...
        if match_type == 'placeholder':
            ...
            text = match.group(1)
            run = paragraph.add_run(sanitize_text_for_word(text))
...
def html_to_marked_text(text: str) -> str:
    ...
    # CRITICAL: Append double newline to ensure split_blocks treats each item as a separate block
    # This aligns with formatFinalArticleWithBlockTypes which sends separate block_type for each item
    ...
    # Convert mark tags to == == markers for highlighting
    text = re.sub(r"<\s*/\s*mark\s*>", "==", text, flags=re.IGNORECASE)
    text = re.sub(r"<\s*mark[^>]*>", "==", text, flags=re.IGNORECASE)
...
def parse_html_to_word_structure(doc: Document, html_content: str):
    """
    Parse HTML content directly to Word document structure.
    Handles the HTML output from formatFinalArticleWithBlockTypes:
    - <h1> → Heading 1 style
    - <h2> through <h6> → Heading 2-6 styles
    - <ul><li> → List Bullet style (grouped)
    - <p> → Body Text style
    - <strong> → bold text
    
    Preserves spacing from inline styles where applicable.
    """
    if not html_content or not html_content.strip():
        return
    
    # Remove wrapper divs if present (from frontend display)
    html_content = re.sub(r'^<div[^>]*>', '', html_content, flags=re.IGNORECASE | re.MULTILINE)
    html_content = re.sub(r'</div>$', '', html_content, flags=re.IGNORECASE | re.MULTILINE)
    
    # Use regex to extract block-level elements (h1-h6, ul, p)
    # Pattern to match complete block elements with their content
    # Note: formatFinalArticleWithBlockTypes produces clean HTML without nested block-level tags
    ...
    for match in block_pattern.finditer(html_content):
        # Add any text before this block as paragraph if it exists (shouldn't happen with clean HTML)
        if match.start() > last_end:
            before_text = html_content[last_end:match.start()].strip()
            # Only add if it's actual text (not just whitespace or HTML fragments)
            if before_text and not re.match(r'^<[^>]+>$', before_text):
                blocks.append({'tag': 'p', 'content': before_text})
    ...
    # Add any remaining text (shouldn't happen with clean HTML from formatFinalArticleWithBlockTypes)
    if last_end < len(html_content):
        remaining = html_content[last_end:].strip()
        if remaining and not re.match(r'^<[^>]+>$', remaining):
            blocks.append({'tag': 'p', 'content': remaining})
    
    # Process blocks and add to Word document
    for block in blocks:
        tag = block['tag']
        ...
        if tag == 'h1':
            ...
            p = doc.add_paragraph(style="Heading 1")
            text = _extract_text_from_html(content, preserve_bold=True)
            ...
            # Parse list items from <ul><li> structure
            li_pattern = re.compile(r'<li[^>]*>(.*?)</li>', re.IGNORECASE | re.DOTALL)
            li_matches = li_pattern.findall(content)
            
            for li_content in li_matches:
                li_text = _extract_text_from_html(li_content, preserve_bold=True)
                para = doc.add_paragraph(style="List Bullet")
                _add_text_with_bold(para, li_text)
                para.paragraph_format.space_after = DocxPt(3)  # ~0.375em
        
        elif tag == 'p' or not tag:
            # Paragraph
            para = doc.add_paragraph(style="Body Text")
            text = _extract_text_from_html(content, preserve_bold=True)
            _add_text_with_bold(para, text)
            para.paragraph_format.space_before = DocxPt(2)   # ~0.15em
            para.paragraph_format.space_after = DocxPt(8)     # ~0.7em

def _extract_text_from_html(html: str, preserve_bold: bool = False) -> str:
    """
    Extract text from HTML, optionally preserving bold markers.
    Returns plain text with ** markers if preserve_bold=True, otherwise plain text.
    """
    if not html:
        return ""
    ...
    if preserve_bold:
        # Convert <strong> and <b> to ** markers
...
def _add_text_with_bold(para, text: str):
    """
    ...
    # Split by ** markers to handle bold text
    ...
    for part in parts:
        if part == '**':
            ...
            run = para.add_run(sanitize_text_for_word(part))
            if bold:
                run.bold = True

def parse_html_to_pdf_structure(story: list, html_content: str, body_style: ParagraphStyle, heading_style: ParagraphStyle):
    """
    Parse HTML content directly to PDF structure.
    Handles the HTML output from formatFinalArticleWithBlockTypes:
    - <h1> → Title style (24pt, bold, orange)
    - <h2> through <h6> → Heading styles (14pt base, bold, orange)
    - <ul><li> → ListFlowable (grouped)
    - <p> → Body Text style
    - <strong> → bold text
    ...
    # Process blocks and add to PDF story
    for i, block in enumerate(blocks):
        ...
        if tag == 'h1':
            ...
            text = _extract_text_from_html(content, preserve_bold=False)
            text = _format_content_for_pdf(text)
            title_style = ParagraphStyle(
                'PWCTitle',
            ...
            list_items = []
            for li_content in li_matches:
                li_text = _extract_text_from_html(li_content, preserve_bold=False)
                # Handle bold text in list items (convert <strong> to <b> for PDF)
                li_text = re.sub(r'<strong>(.*?)</strong>', r'<b>\1</b>', li_text, flags=re.IGNORECASE)
                li_text = _format_content_for_pdf(li_text)
                list_items.append(ListItem(Paragraph(li_text, body_style)))
            
            if list_items:
                # Determine spacing
                space_before = 3 if prev_block and prev_block.get('tag') == 'p' else 6
                space_after = 3 if next_block and next_block.get('tag') == 'p' else 6
            ...
            if next_block and next_block.get('tag') == 'ul':
                para_style.spaceAfter = 3  # ~0.25em
            # Reduce spacing if following heading
            if prev_block and prev_block.get('tag', '').startswith('h'):
                para_style.spaceBefore = 0
            
            text = _extract_text_from_html(content, preserve_bold=False)
            # Handle bold text (convert <strong> to <b> for PDF)
            text = re.sub(r'<strong>(.*?)</strong>', r'<b>\1</b>', text, flags=re.IGNORECASE)
            text = _format_content_for_pdf(text)
            story.append(Paragraph(text, para_style))

# Removed _format_content_with_block_types_word - no longer needed since HTML is always sent from frontend
...
def export_to_word_edit_content(
    ...
    Parses HTML content directly to preserve structure (h1-h6, p, ul, li).
    Includes Table of Contents with numbered headings.
    
    Note: Frontend always sends formatted HTML content (from formatFinalArticleWithBlockTypes).
    """
    # Check if content is HTML (from formatFinalArticleWithBlockTypes)
    # Use specific tag detection for reliable HTML identification
    html_pattern = re.compile(r'<(h[1-6]|p|ul|li|div|strong|em|mark|br)[^>]*>', re.IGNORECASE)
    ...
    if is_html:
        ...
        h_pattern = re.compile(r'<h([1-6])[^>]*>(.*?)</h\1>', re.IGNORECASE | re.DOTALL)
        for match in h_pattern.finditer(content):
            heading_text = _extract_text_from_html(match.group(2), preserve_bold=False).strip()
            heading_text = re.sub(r'\*+', '', heading_text).strip()
            if heading_text and heading_text.lower() not in ["references", "references:"]:
                headings.append(heading_text)
    else:
        logger.warning(f"[Export Word Edit] Content is not HTML. Expected HTML from formatFinalArticleWithBlockTypes.")
    ...
    if "references" in content.lower() or "references:" in content.lower():
        ...
        if is_html:
            # Parse HTML directly
            parse_html_to_word_structure(doc, main_content)
        else:
            logger.warning(f"[Export Word Edit] Content is not HTML. Cannot parse structure.")
        ...
        if len(parts) > 1:
            ...
            if is_html:
                parse_html_to_word_structure(doc, parts[1])
    else:
        if is_html:
            # Parse HTML directly (preserves formatFinalArticleWithBlockTypes structure)
            parse_html_to_word_structure(doc, content)
        else:
            logger.warning(f"[Export Word Edit] Content is not HTML. Cannot parse structure.")
...
# Removed _format_content_with_block_types_pdf - no longer needed since HTML is always sent from frontend

def _format_content_with_block_types_pdf_removed(story: list, content: str, block_types: list[dict] | None = None, 
    ...
    # Create block type map
    block_type_map = {bt.get("index", i): bt for i, bt in enumerate(block_types)}
    
    # Use normalized split_blocks instead of content.split("\n\n")
    paragraphs = split_blocks(content)
    
    # STRICT MODE: Verify block_types alignment
    if len(block_types) < len(paragraphs):
        raise ValueError(
            f"Block type mismatch in PDF export: {len(block_types)} block_types provided, "
            f"but content has {len(paragraphs)} blocks. "
            f"This indicates upstream pipeline corruption."
        )
    ...
    for idx, block in enumerate(paragraphs):
        ...
        if not block_info:
            # Strict mode: fail instead of guessing
            raise ValueError(
                f"Block type missing for paragraph index {idx} in PDF export. "
                f"Expected block_types[{idx}] to exist. "
                f"Total blocks: {len(paragraphs)}, provided block_types: {len(block_types)}"
            )
        ...
        if block_type == 'bullet_item':
            ...
            if block_type == "title":
                ...
                story.append(para)
                 ...
                 # Unknown block type (fallback to paragraph): proper spacing
                para_style = ParagraphStyle(
                    'PWCBodyPara',
                    parent=body_style,
                    spaceAfter=8,   # ~0.7em
...
# Removed _format_content_with_block_types_pdf function (lines 3676-4092) - no longer needed since HTML is always sent from frontend
...
def export_to_pdf_edit_content(
    ...
    if is_html:
        ...
        logger.warning(f"[Export PDF Edit] Content is not HTML. Expected HTML from formatFinalArticleWithBlockTypes.")
    ...
    if is_html:
        # Parse HTML directly (preserves formatFinalArticleWithBlockTypes structure)
        parse_html_to_pdf_structure(story, content, body_style, heading_style)
    else:
        logger.warning(f"[Export PDF Edit] Content is not HTML. Cannot parse structure.")
