  ...
  private getBlockTypesForExport(metadata: any, content: string): { content: string; blockTypes: BlockTypeInfo[] } {
    ...
    if (blockTypes && Array.isArray(blockTypes) && blockTypes.length > 0) {
      ...
      if (blockTypes.length === contentBlocks.length) {
        ...
        console.warn(`[Chat Export] BlockTypes count (${blockTypes.length}) doesn't match content blocks (${contentBlocks.length}), regenerating...`);
      }
    }
    ...
    if (Array.isArray(paragraphEdits) && paragraphEdits.length > 0) {
      ...
      blockTypes = contentBlocks.map((block: string, idx: number) => {
      ...
      console.log('[Chat Export] Regenerated blockTypes from paragraphEdits (same logic as backend):', blockTypes);
      return { content: plainTextContent, blockTypes };
    }
    
    // Last fallback: all paragraphs
    const contentBlocks = plainTextContent.split(/\n\n+/).filter(b => b.trim());
    blockTypes = contentBlocks.map((_: string, idx: number) => ({ 
      index: idx, 
      type: 'paragraph', 
      level: 0 
    } as BlockTypeInfo));
    console.warn('[Chat Export] Using default blockTypes (all paragraph):', blockTypes);
    return { content: plainTextContent, blockTypes };
  }
