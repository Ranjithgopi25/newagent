...
def planner_node(state: RefineGraphState):
    ...
    if apply_research:
        ...
        # Expand: use expand guidelines as research topic when none set (e.g. user typed "insurance")
        if research_input and is_expand:
            guidelines = (services.get("supporting_doc_instructions") or "").strip()
            topics = (research_input.get("research_topics") or "").strip()
            if guidelines:
                research_input["research_topics"] = f"{topics}; {guidelines}".strip("; ") if topics else guidelines
    else:
        research_input = None
