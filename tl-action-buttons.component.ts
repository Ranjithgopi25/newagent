...
class IntentDetectionService:
    ...
    def _detect_editors_from_input(self, user_input: str) -> List[str]:
        """Direct pattern matching for editor names in user input."""
        user_lower = user_input.lower()
        detected = []
        
        editor_patterns = {
            "content": ["content editor", "content editing", "content edit", " content "],
            "line": ["line editor", "line editing", "line edit", " line "],
            "copy": ["copy editor", "copy editing", "copy edit", " copy "],
            "development": ["development editor", "development editing", "development edit"],
            "brand-alignment": ["brand alignment", "brand-alignment", "brand editor"]
        }
        
        for editor_id, patterns in editor_patterns.items():
    ...
    async def detect_edit_intent(self, user_input: str) -> Dict[str, any]:
        ...
        """
        # Pre-check: Direct pattern matching for editor names (outside try block for error handling)
        direct_editors = self._detect_editors_from_input(user_input)
        direct_has_edit_intent = len(direct_editors) > 0 or any(keyword in user_input.lower() for keyword in ["edit", "editing", "edited"])
        
        try:
            system_prompt = """YOU ARE AN INTENT CLASSIFICATION AGENT.

YOUR TASK:
Detect **EDIT INTENT** when the user input contains:
1. The word "edit", "editing", or "edited" (case-insensitive)
OR
2. An editor name (see EDITOR NAMES below)
...
━━━━━━━━━━━━━━━━━━━━
EDITOR NAMES (TRIGGER EDIT INTENT)
━━━━━━━━━━━━━━━━━━━━

The following editor names/patterns trigger edit intent:
- "line editor" or "line"
- "copy editor" or "copy"
- "development editor" or "development"
- "content editor" or "content"
- "brand-alignment" or "brand alignment" or "brand"

If user mentions ANY of these editor names, set `"is_edit_intent": true`
...
2. IF the input text contains an editor name (see EDITOR NAMES above)
   → set `"is_edit_intent": true`

3. IF NONE of these appear
   → set `"is_edit_intent": false`

4. DO NOT infer meaning, intent, or user goals beyond literal matching.
5. DO NOT use semantic similarity.
...
UNLESS the word "edit", "editing", "edited" OR an editor name is ALSO present:
...
and does NOT contain "edit", "editing", "edited", or an editor name
→ `"is_edit_intent": false`

━━━━━━━━━━━━━━━━━━━━
EDITOR DETECTION (ALWAYS CHECK)
━━━━━━━━━━━━━━━━━━━━

Check whether the user EXPLICITLY mentions any of the following editors:

- "line" (matches "line editor", "line editing", "line edit", etc.)
- "copy" (matches "copy editor", "copy editing", "copy edit", etc.)
- "development" (matches "development editor", "development editing", etc.)
- "content" (matches "content editor", "content editing", "content edit", etc.)
- "brand-alignment" or "brand alignment" or "brand" (matches "brand alignment editor", "brand editor", etc.)
...
- NEVER add text outside the JSON response
        ...
        """
            ...
            if json_start != -1 and json_end > json_start:
                ...
                llm_is_edit_intent = result.get("is_edit_intent", False)
                ...
                # Validate and map LLM-detected editors to valid IDs
                valid_editor_ids = ["line", "copy", "development", "content", "brand-alignment"]
                llm_detected_editors = []
                    ...
                    for editor in detected_editors_raw:
                        ...
                        for valid_id in valid_editor_ids:
                            if editor_lower == valid_id or editor_lower.replace(" ", "-") == valid_id:
                                if valid_id not in llm_detected_editors:
                                    llm_detected_editors.append(valid_id)
                                break
                
                # Merge direct detection with LLM detection (direct takes priority)
                detected_editors = list(set(direct_editors + llm_detected_editors))
                
                # Use direct detection if found, otherwise use LLM
                if len(direct_editors) > 0:
                    is_edit_intent = True
                    confidence = max(0.9, llm_confidence)
                elif direct_has_edit_intent:
                    is_edit_intent = True
                    confidence = max(0.8, llm_confidence)
                else:
                    is_edit_intent = llm_is_edit_intent
                    confidence = llm_confidence
                
                logger.info(f"Intent detection: is_edit_intent={is_edit_intent}, confidence={confidence:.2f}, editors={detected_editors}")
                ...
                # Fallback if JSON parsing fails - use direct detection
                logger.warning(f"Failed to parse JSON from LLM response: {response_text}")
                return {
                    "is_edit_intent": direct_has_edit_intent,
                    "confidence": 0.8 if len(direct_editors) > 0 else 0.6,
                    "reasoning": f"Direct pattern matching (JSON parsing failed). Editors: {direct_editors}",
                    "detected_editors": direct_editors
                }
                    
        except Exception as e:
            logger.error(f"Error in intent detection: {str(e)}", exc_info=True)
            # Use direct detection as fallback
            return {
                "is_edit_intent": direct_has_edit_intent,
                "confidence": 0.8 if len(direct_editors) > 0 else 0.6,
                "reasoning": f"Direct pattern matching (Error: {str(e)}). Editors: {direct_editors}",
                "detected_editors": direct_editors
            }
