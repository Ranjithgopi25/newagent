    ...
    while pos < len(text_with_placeholders):
        ...
        if match_type == 'placeholder':
            ...
            if placeholder_text in link_placeholders:
                ...
                # Citation refs [1](url), [¹](url): make clickable superscript (preserve inline URL in export)
                superscript_digits = "⁰¹²³⁴⁵⁶⁷⁸⁹"
                is_citation = link_text.isdigit() or (len(link_text.strip()) <= 3 and any(c in superscript_digits for c in link_text))
                if is_citation:
...
def _normalize_citation_html_to_markdown(content: str) -> str:
    """Convert citation HTML to markdown so inline URLs are preserved in export and UI.
    Replaces <sup>[ [¹](URL) ]</sup> with [¹](URL) so markdown link is kept (clickable in Word/PDF and UI).
    """
    if not content or "<sup>" not in content:
        return content
    # Match <sup>[ [¹](URL) ]</sup> or similar; URL is everything between )( and )
    pattern = re.compile(
        r"<sup>\s*\[\s*\[([⁰¹²³⁴⁵⁶⁷⁸⁹,\s\[\]]+)\]\((https?://[^)]+)\)\s*\]\s*</sup>",
        re.IGNORECASE,
    )
...
def export_to_word_edit_content(
    ...
    block_types should match final article generation structure with sequential indices.
    """
    # Normalize citation HTML to markdown so inline URLs are preserved (clickable in Word)
    content = _normalize_citation_html_to_markdown(content)
...
def export_to_pdf_edit_content(
    ...
    block_types should match final article generation structure with sequential indices.
    """
    # Normalize citation HTML to markdown so inline URLs are preserved (clickable in PDF)
    content = _normalize_citation_html_to_markdown(content)
    # Content should be plain text final_article (from backend final article generation)
    # Format: "\n\n".join(final_paragraphs) - same as final article generation
    # Safety check: convert HTML to plain text if somehow HTML is present
