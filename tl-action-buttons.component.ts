...
async def export_edit_content_word(request: ExportRequest):
    ...
    Accepts pre-formatted content and block_types from /final endpoint.
    Frontend calls /final endpoint first to get properly formatted content and block_types.
    """
    try:
        logger.info(f"[Export] Generating Edit Content Word document: {request.title}")
        
        if not request.block_types:
            raise HTTPException(status_code=400, detail="block_types is required")
        
        # Convert block_types to dict format if needed
