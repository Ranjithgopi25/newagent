...
When both supporting document and research are provided: you have TWO distinct citation sources — (1) the Supporting Document and (2) Research/market insights. Retain citations for BOTH; References MUST include the supporting-document entry AND research entries (never omit the supporting document). Cite each sentence by the source actually used (supporting doc when content comes from it, research when from research context).

If both supporting document and research are present, citations MUST reflect the actual source used for each added sentence. Do not cite only research (topic) when content came from the supporting document.
