...
def _add_list_to_pdf(story: list, list_items: list[dict], list_type: str, body_style: ParagraphStyle, 
    ...
    Numbered lists (list_type=='number') use decimal numbering (1, 2, 3...).
    Bullet and other list types use bullet icons.
    """
    if not list_items:
        return
    ...
    # Create list items; strip number/letter/bullet prefix so we don't duplicate (ReportLab adds numbers for numbered lists)
    pdf_list_items = []
    ...
    for item in list_items:
        ...
        # Remove prefix: numbered lists get numbers from ReportLab; bullets get bullet from ReportLab
        clean_content = content
        if list_type == 'number':
            clean_content = re.sub(r'^\d+\.\s+', '', content.strip())
        elif list_type in ['alpha_upper', 'alpha_lower']:
    ...
    # Numbered lists use decimal (1, 2, 3...); bullet lists use bullet icons
    ...
    for i, block_info in enumerate(formatted_blocks):
        ...
        if block_type == 'list_item' or block_type == 'bullet_item':
            ...
            if block_type == "heading":
                # Preserve heading numbers (e.g. "1.", "2.") from backend so Word shows correct order
                # Heading: Based on level (Heading 1-6), bold, font-weight 600 equivalent
                heading_level = min(max(level, 1), 6)
                style_name = f"Heading {heading_level}"
                p = doc.add_paragraph(style=style_name)
                
                # Remove numbering from heading (avoid Word outline numbering; we use text "1.", "2.")
...
def _format_content_with_block_types_pdf(story: list, content: str, block_types: list[dict] | None = None, 
    ...
    for i, block_info in enumerate(formatted_blocks):
        ...
        if block_type == 'list_item' or block_type == 'bullet_item':
            ...
            if block_type == "heading":
                # Preserve heading numbers (e.g. "1.", "2.") from backend so PDF shows correct order
...
def export_to_word_pwc_no_toc(
    ...
    # for raw_line in content.split("\n\n"):
        ...
        #     clean = clean.replace("*", "")
        #     p = doc.add_paragraph(clean, style="Body Text")
        # if re.fullmatch(r"-{2,}", line):
        
        # logger.info(f"Processing line for Word export: {line}")
        ...
        #     # Bold bullet heading if present
        #     if "**" in text:
...
#     if doc.paragraphs:
#         doc.paragraphs[0].text = clean_title
#     if subtitle and len(doc.paragraphs) > 1:
#         doc.paragraphs[1].text = subtitle

#     doc.add_page_break()

#     # 3. Add generated response directly
...
#     # 4. Save to memory (same as old behavior)
#     buffer = io.BytesIO()
#     doc.save(buffer)
#     buffer.seek(0)
