...
// Ensure flow container matches other modals exactly
.flow-container {
  width: 90%;
  max-width: 560px;
  max-height: 90vh;
...
.editor-toggle-item {
  ...
  &:disabled + .editor-toggle-switch {
    background: #D04A02;  // Exact same color as checked state
    color: white;
    cursor: not-allowed;
...
.editor-timeline {
  ...
  .timeline-item.active.loading .timeline-marker {
  ...
  .timeline-item.active.loading .timeline-status {
    animation: blink 1s infinite;
  }
