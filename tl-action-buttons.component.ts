block_type_info += "Heading mapping is mandatory: type='title' -> '#', type='heading' level=1 -> '##', level=2 -> '###', level=3 -> '####'. If heading text starts with numbering/lettering (e.g., '1. Introduction', 'A. Scope'), keep that prefix inside the heading text (example: '## 1. Introduction').\n"
        # Prepare m
...
- Standalone numbered section headings (e.g., "1. Introduction", "2. Market Trends")
  are headings, not list items. Format them as markdown headings using the correct heading level
  while preserving the numeric prefix text (example: `# 1. Introduction`).
...
- Main sections/subheadings: ## Heading; deeper sections: ### and ####
- Body: normal paragraphs. Single blank line between blocks.
- If a standalone line is a section heading that starts with numbering/lettering
  (e.g., "1. Introduction", "A. Scope", "I. Overview"), render it as a heading with `##`/`###`/`####`
  as appropriate, and keep the original prefix text intact.
...
3. Exactly one level-1 heading (# Title) is present; any extra title-like lines have been
   converted into subheadings (## ...) or removed if clearly unnecessary.
