...
private exportWordNewLogic(): void {
  ...
  private getBlockTypesForExport(): { content: string; blockTypes: BlockTypeInfo[] } {
    ...
    let plainTextContent = this.metadata.fullContent;
    ...
    if (Array.isArray(paragraphEdits) && paragraphEdits.length > 0) {
      ...
      console.log('[Export] Regenerated blockTypes from paragraphEdits (same logic as backend):', blockTypes);
    ...
    console.warn('[Export] Using default blockTypes (all paragraph):', blockTypes);
    return { content: plainTextContent, blockTypes };
  }
