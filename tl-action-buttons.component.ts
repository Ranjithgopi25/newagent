...
class SupervisorState(TypedDict):
    ...
    dev_editor_retry_count: Optional[int]  # Unused: validation is initial only once, no retries
    content_editor_retry_count: Optional[int]  # Unused: validation is initial only once, no retries
    ...
    content_input_blocks: Optional[List[Any]]  # When development+content: blocks from dev result for content editor
...
def content_editor_node(state: SupervisorState) -> SupervisorState:
    ...
    # When in development+content flow, run content on dev output (content_input_blocks)
    input_blocks = state.get("content_input_blocks") or state["document"].blocks
    if state.get("content_input_blocks"):
        logger.info("Using content_input_blocks (development output) for Content Editor")
...
def content_editor_retry_node(state: SupervisorState) -> SupervisorState:
    ...
    # Use same input as content_editor_node: content_input_blocks when in dev+content flow, else document
    input_blocks = state.get("content_input_blocks") or state["document"].blocks
    result = run_editor_engine(
        "content",
        input_blocks,
...
def cross_paragraph_analysis_node(state: SupervisorState) -> SupervisorState:
    ...
    When in development+content flow, analyzes dev result and sets content_input_blocks.
    """
    ...
    # development+content: analyze dev result and set content_input_blocks so content runs on dev output
    if (
        current_idx < len(selected_editors)
        and selected_editors[current_idx] == "development+content"
        and editor_results
        and editor_results[-1].editor_type == "development"
    ):
        dev_result = editor_results[-1]
        doc_from_dev = DocumentStructure(
            blocks=[
                DocumentBlock(
                    id=b.id,
                    type=b.type,
        ...
        analysis = analyze_cross_paragraph_logic(doc_from_dev)
        content_input_blocks = [
            DocumentBlock(
                id=b.id,
                type=b.type,
                level=b.level,
        ...
        return {
            ...
            "content_input_blocks": content_input_blocks,
        }
...
def content_validation_node(state: SupervisorState) -> SupervisorState:
    ...
    # When content ran on dev output (development+content flow), use that as original for validation
    original_doc = (
        DocumentStructure(blocks=state["content_input_blocks"])
        if state.get("content_input_blocks")
        else state["document"]
    )
...
def merge_node(state: SupervisorState) -> SupervisorState:
    ...
    editor_attr_map = {
        ...
        # combined steps
        "development+content": "development_content",
        "line+copy": "line_copy",
    }
...
# ---------------------------------------------------------------------
# COMBINE DEVELOPMENT + CONTENT NODE (single EditorResult for step)
# ---------------------------------------------------------------------
def combine_development_content_node(state: SupervisorState) -> SupervisorState:
    """
    Merge the last two EditorResults (development, content) into one
    EditorResult(editor_type="development+content") with combined feedback per block.
    """
    logger.info("RUNNING: combine_development_content_node")
    editor_results = list(state.get("editor_results", []))
    if len(editor_results) < 2:
        logger.error("combine_development_content requires at least 2 editor results")
        return {"content_input_blocks": None}
    
    content_result = editor_results[-1]
    dev_result = editor_results[-2]
    if content_result.editor_type != "content" or dev_result.editor_type != "development":
        logger.error("Expected last two results to be development and content")
        return {"content_input_blocks": None}
    
    # Build block id -> (dev_block, content_block)
    dev_by_id = {b.id: b for b in dev_result.blocks}
    content_by_id = {b.id: b for b in content_result.blocks}
    
    merged_blocks: List[BlockEditResult] = []
    for blk in content_result.blocks:
        dev_blk = dev_by_id.get(blk.id)
        content_blk = blk
        # Combined feedback: all FeedbackItems from dev and content for this block
        all_items: List[FeedbackItem] = []
        if dev_blk and dev_blk.feedback_edit:
            for sef in dev_blk.feedback_edit:
                if sef.editor == "development":
                    all_items.extend(sef.items)
        if content_blk.feedback_edit:
            for sef in content_blk.feedback_edit:
                if sef.editor == "content":
                    all_items.extend(sef.items)
        merged_feedback = (
            [SingleEditorFeedback(editor="development+content", items=all_items)]
            if all_items
            else []
        )
        merged_blocks.append(
            BlockEditResult(
                id=content_blk.id,
                type=content_blk.type,
                level=content_blk.level,
                original_text=content_blk.original_text,
                suggested_text=content_blk.suggested_text,
                has_changes=content_blk.has_changes,
                feedback_edit=merged_feedback,
            )
        )
    
    combined_result = EditorResult(
        editor_type="development+content",
        blocks=merged_blocks,
        warnings=[],
    )
    # Append combined result (state uses operator.add for editor_results; sequential_merge uses last)
    return {
        "editor_results": [combined_result],
        "content_input_blocks": None,
    }


# ---------------------------------------------------------------------
# LINE + COPY COMBINED NODE (runs line then copy, single EditorResult)
# ---------------------------------------------------------------------
def line_copy_combined_node(state: SupervisorState) -> SupervisorState:
    """
    Run line editor then copy editor (copy on line output), merge into
    one EditorResult(editor_type="line+copy") with combined feedback.
    """
    logger.info("RUNNING: line_copy_combined_node")
    input_blocks = state["document"].blocks
    
    # 1. Run line editor
    raw_line = line_editor_tool.invoke({"blocks": input_blocks})
    line_result = normalize_editor_output("line", raw_line)
    
    # 2. Build blocks from line result for copy
    line_blocks = [
        DocumentBlock(
            id=b.id,
            type=b.type,
    ...
    # 3. Run copy editor on line output
    raw_copy = copy_editor_tool.invoke({"blocks": line_blocks})
    copy_result = normalize_editor_output("copy", raw_copy)
    
    # 4. Merge: one EditorResult with combined feedback per block
    line_by_id = {b.id: b for b in line_result.blocks}
    copy_by_id = {b.id: b for b in copy_result.blocks}
    merged_blocks_list: List[BlockEditResult] = []
    for blk in copy_result.blocks:
        line_blk = line_by_id.get(blk.id)
        copy_blk = blk
        all_items = []
        if line_blk and line_blk.feedback_edit:
            for sef in line_blk.feedback_edit:
                if sef.editor == "line":
                    all_items.extend(sef.items)
        if copy_blk.feedback_edit:
            for sef in copy_blk.feedback_edit:
                if sef.editor == "copy":
                    all_items.extend(sef.items)
        merged_feedback = (
            [SingleEditorFeedback(editor="line+copy", items=all_items)]
            if all_items
            else []
        )
        merged_blocks_list.append(
            BlockEditResult(
                id=copy_blk.id,
                type=copy_blk.type,
                level=copy_blk.level,
                original_text=copy_blk.original_text,
                suggested_text=copy_blk.suggested_text,
                has_changes=copy_blk.has_changes,
                feedback_edit=merged_feedback,
            )
        )
    
    combined_result = EditorResult(
        editor_type="line+copy",
        blocks=merged_blocks_list,
        warnings=[],
    )
    # State uses operator.add for editor_results; return delta only
    return {"editor_results": [combined_result]}
...
def route_sequential_editor(state: SupervisorState):
    ...
    # development and development+content: same flow (article_analysis -> dev -> validation -> then cross_paragraph/content for +content)
    if editor_name in ("development", "development+content"):
        article_analysis = state.get("article_analysis")
        editor_results = state.get("editor_results", [])
    ...
    # Combined step: line + copy in one node
...
# ---------------------------------------------------------------------
# VALIDATION & RETRY — BEFORE VS AFTER
# ---------------------------------------------------------------------
# BEFORE: Validation ran after Development and Content editors; if score < 8
#         we retried up to MAX_RETRIES (2) times before proceeding to merge.
# AFTER:  Validation runs once only (initial run). No retries. If score < 8
#         we proceed immediately to merge / cross_paragraph_analysis /
#         combine_development_content. User sees the single validated result.
# ---------------------------------------------------------------------
...
def route_after_content_validation(state: SupervisorState) -> str:
    """After validation: initial only once, no retries. Score >= 8 -> merge/combine; else -> merge."""
    validation_result = state.get("content_validation_result")
    retry_count = state.get("content_editor_retry_count", 0)
    MAX_RETRIES = 0  # Initial only once: no retries
    current_idx = state.get("current_editor_index", 0)
    selected_editors = state.get("selected_editors", [])
    current_editor = selected_editors[current_idx] if current_idx < len(selected_editors) else None
    
    if validation_result:
        score = validation_result.score
        logger.info(f"Content validation score: {score}/10 (initial only once, no retries)")
        
        # Merge if score >= 8 (passing threshold)
        if score >= 8:
            logger.info(f"Score {score} >= 8, proceeding")
            # development+content: combine dev and content into one result then merge
            if current_editor == "development+content":
                return "combine_development_content"
            return "merge"
        
        # No retries: proceed to merge/combine regardless of score
        logger.info(f"Score {score} < 8, proceeding without retry (initial only once)")
        if current_editor == "development+content":
            return "combine_development_content"
        return "merge"
...
# ---------------------------------------------------------------------
# ROUTER AFTER VALIDATION (DEVELOPMENT EDITOR)
# ---------------------------------------------------------------------
def route_after_validation(state: SupervisorState) -> str:
    """After validation: initial only once, no retries. Score >= 8 -> merge/cross_paragraph; else -> merge."""
    validation_result = state.get("validation_result")
    retry_count = state.get("dev_editor_retry_count", 0)
    MAX_RETRIES = 0  # Initial only once: no retries
    current_idx = state.get("current_editor_index", 0)
    ...
    if validation_result:
        score = validation_result.score
        logger.info(f"Validation score: {score}/10 (initial only once, no retries)")
        
        # Merge if score >= 8 (passing threshold)
        if score >= 8:
            logger.info(f"Score {score} >= 8, proceeding")
            # development+content: continue to cross-paragraph analysis then content
            if current_editor == "development+content":
                return "cross_paragraph_analysis"
            return "merge"
        
        # No retries: proceed to merge or cross_paragraph regardless of score
        logger.info(f"Score {score} < 8, proceeding without retry (initial only once)")
        if current_editor == "development+content":
            return "cross_paragraph_analysis"
        return "merge"
...
def build_sequential_graph():
    ...
    graph.add_node("combine_development_content", combine_development_content_node)
    ...
    graph.set_conditional_entry_point(
        route_sequential_editor,
        {
            ...
            "combine_development_content": "combine_development_content",
    ...
    graph.add_edge("combine_development_content", "merge")
