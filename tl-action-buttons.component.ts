...
def build_compression_prompt(
    ...
    is_extreme_reduction = reduction_percentage > 50  # For reductions >50%, allow paragraph deletion
...
- This is an EXTREME compression requiring drastic measures
- You MUST delete entire paragraphs if they are less critical
- You MUST remove entire sections that are not essential to core arguments
- Combine multiple paragraphs into single paragraphs
- Delete ALL examples, case studies, and supporting details
- Keep ONLY the absolute core arguments and essential facts
- Remove ALL transitional paragraphs and introductory sections
- Eliminate entire paragraphs that repeat or restate main points
- This level of reduction REQUIRES structural changes, not just word-level compression
...
- Consider deleting less critical paragraphs if needed
""".format(reduction_percentage)
...
- Apply MAXIMUM compression techniques:
  * DELETE entire paragraphs that are less critical or repetitive
  * DELETE entire sections that don't contain core arguments
  * Combine multiple paragraphs into single paragraphs
  ...
  * Compress lists to their most essential points (or remove lists entirely)
  * Remove redundant explanations and restatements
  * Use the most concise phrasing possible for every concept
  * DELETE introductory paragraphs that don't add unique value
  * DELETE concluding paragraphs that only summarize
- Structural deletion is REQUIRED at this stage - word-level compression alone is insufficient
""".format(retry_count)
    
    # Build structural requirements based on reduction size and retry count
    structural_requirements = ""
    if is_extreme_reduction or retry_count >= 2:
        structural_requirements = """- For EXTREME reductions (>50%): You MAY delete entire paragraphs and sections that are less critical. Prioritize word count over preserving all paragraphs.
- Do NOT add new content
- Maintain logical flow and coherence
- Structural deletion is REQUIRED to meet word count target"""
    elif (is_very_large_reduction or is_extreme_reduction) and retry_count == 0:
        structural_requirements = """- For VERY LARGE reductions (>45%): You MAY delete less critical paragraphs if necessary to meet word count.
- Do NOT add new content
- Maintain logical flow and coherence
- Preserve paragraph structure (but compress content within) - unless deletion is necessary"""
    else:
...
7. PARAGRAPH-LEVEL COMPRESSION:
   - Compress WITHIN each paragraph
   ...
   - For extreme reductions (>50%) or retries: DELETE entire paragraphs that are less critical or repetitive
...
- If reduction is >50%: Structural deletion (paragraphs/sections) is REQUIRED
- If retry_count >= 2: Structural deletion is MANDATORY - word-level compression is insufficient
- If you are not meeting the target, you MUST apply more aggressive compression techniques
- DO NOT prioritize "preserving all paragraphs" over meeting the word count target
"""
        },
        {
            "role": "user",
