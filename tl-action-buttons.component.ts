...
.mcx-banner {
  ...
  &::before {
    ...
    height: 100%;
    transform: translateY(-50%);
    background: linear-gradient(to top right, #fcf8f8 0%, #FDEBED 25%, #f9e6db 50%, #FFD4C4 90%, #fac7b6 100%);
    z-index: 1;
    opacity: 1;
  ...
  .banner-content {
    ...
    .banner-title {
      font-size: 40px;
      font-weight: 700;
      font-family: "ITC Charter", "Charter", serif;
      color: black;
      margin: 0;
      ...
      // Word break for Market Intelligence and Doc Studio banners which have longer text
      &.market-intelligence {
        white-space: normal;
        // word-break: break-word;
      ...
      &.ppt {
        ...
        // At screen size >= 2100px, keep at 80% width for wrapping
...
.feature-name {
  ...
  box-shadow: 0px 4px 10px rgba(254, 124, 57);
}
...
.top-action-btn {
  ...
  &.primary {
    ...
    &:hover {
      //background: linear-gradient(135deg, #B63E00 0%, #FFAA72 100%);
      background-color: #FFAA72 ;
...
/* Final article / revised content: tighter spacing (same as edit-content-flow) */
...
.processing-line {
  ...
  // Animation when line is being removed from sliding window
  &.remove-animate {
    animation: slideUpAndFade 0.6s ease-out forwards;
  }

  // Style for the current processing line (last line with typing dots)
  &.last-line {
    animation: currentMessagePulse 2s ease-in-out infinite;
    background: linear-gradient(90deg, rgba(var(--pwc-orange-rgb), 0.05) 0%, rgba(var(--pwc-orange-rgb), 0) 100%);
...
@keyframes currentMessagePulse {
  0% {
    box-shadow: 0 0 0 0 rgba(var(--pwc-orange-rgb), 0.3);
    transform: scale(1.015);
  }
  50% {
    box-shadow: 0 0 8px 2px rgba(var(--pwc-orange-rgb), 0.15);
    transform: scale(1.0);
  }
  100% {
    box-shadow: 0 0 0 0 rgba(var(--pwc-orange-rgb), 0.3);
...
.message-text {
  ...
  // Tighter spacing when list immediately follows heading (e.g. Citations & References)
...
.action-btn {
  ...
  &.btn-canvas {
    ...
    // Ready to publish button animation states
    &.preparing {
      background-color: var(--bg-tertiary);
      color: #FFAA72;
      border-color: #FFAA72;
    ...
    // Prepared state - show success indicator
    &.prepared {
      background-color: #10b981;
      border-color: #059669;
      color: white;
...
.tl-action-cards-grid {
  ...
  > .tl-action-card:nth-child(4) {
    grid-column: 2 / span 2;
  }

  > .tl-action-card:nth-child(5) {
...
.landing-page-content {
  ...
  position: relative;
  z-index: 1;
  //margin-top: 1.5rem; /* removed negative margin that was causing overlap with banner */
...
.landing-buttons-grid {
    ...
    // Dynamic grid columns based on accessible modules
    &.grid-cols-1 {
...
.ai-disclaimer {
  ...
  font-size: 11.5px;
  color: var(--text-secondary);
  background: rgba(var(--pwc-orange-rgb), 0.08);
  border: 1px solid rgba(var(--pwc-orange-rgb), 0.2);
  border-radius: 6px;
  flex-shrink: 0;
...
// Explore What's Next Box
.explore-whats-next-box {
  width: 100%;
  max-width: 390px;
  padding: 0.20rem 1.5rem 0.5rem 1.5rem;
  margin-top: -1rem;
  border: 1px solid #e6e6e9;
  border-radius: 0px;
  ...
  .explore-icon {
    ...
    height: 48px;
    border-radius: 6px;
    background: linear-gradient(135deg, rgba(255, 159, 0, 0.1) 0%, rgba(194, 65, 12, 0.05) 100%);
  ...
  .explore-text {
    ...
    .explore-subtitle {
      font-size: clamp(0.8rem, 1.1vw, 13px);
      color: var(--text-secondary);
      margin: 0;
      line-height: 1.5;
      font-weight: 500;
...
.explore-showcase-grid {
  ...
  .top-left { grid-column: 1 / span 2; grid-row: 1; }
  .top-center { grid-column: 3 / span 2; grid-row: 1; }
  .top-right { grid-column: 5 / span 2; grid-row: 1; }
  .bottom-left { grid-column: 2 / span 2; grid-row: 2; }
  .bottom-right { grid-column: 4 / span 2; grid-row: 2; }
}
...
.reminder-content {
  ...
  p {
    ...
    .reminder-link {
      ...
      &:hover {
        color: color.scale(#FF8C00, $lightness: -10%);
      }
    }
  }
}
