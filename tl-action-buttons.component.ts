...
from .prompt import COMBINE_EDITORS_SYSTEM_PROMPT
...
# ---------------------------------------------------------------------
# COMBINE EDITORS (LLM merge by same-sentence: rules + impact, final output)
# ---------------------------------------------------------------------
def _run_combine_editors(
    result_a: EditorResult,
    result_b: EditorResult,
    combined_editor_type: str,
    document: DocumentStructure,
) -> EditorResult:
    """Call LLM to merge two editor results by same-sentence rule; return one EditorResult."""
    # Build input JSON for the two editors (block id -> suggested_text and feedback per editor)
    blocks_a = [{"id": b.id, "type": b.type, "level": b.level, "original_text": b.original_text, "suggested_text": getattr(b, "suggested_text", None) or b.original_text, "feedback_edit": [{"editor": sef.editor, "items": [{"issue": i.issue, "fix": i.fix, "impact": i.impact, "rule_used": i.rule_used, "priority": i.priority} for i in sef.items]} for sef in (b.feedback_edit or [])]} for b in result_a.blocks]
    blocks_b = [{"id": b.id, "type": b.type, "level": b.level, "original_text": b.original_text, "suggested_text": getattr(b, "suggested_text", None) or b.original_text, "feedback_edit": [{"editor": sef.editor, "items": [{"issue": i.issue, "fix": i.fix, "impact": i.impact, "rule_used": i.rule_used, "priority": i.priority} for i in sef.items]} for sef in (b.feedback_edit or [])]} for b in result_b.blocks]
    payload = {"editor_a": result_a.editor_type, "editor_b": result_b.editor_type, "blocks_a": blocks_a, "blocks_b": blocks_b}
    user_content = json.dumps(payload, indent=2)

    response = llm.invoke([
        SystemMessage(content=COMBINE_EDITORS_SYSTEM_PROMPT),
        HumanMessage(content=user_content),
    ])
    content = response.content if hasattr(response, "content") else str(response)
    # Parse JSON (allow wrapped in markdown or text)
    json_match = re.search(r"\{[\s\S]*\"blocks\"[\s\S]*\}", content)
    if json_match:
    ...
    blocks_out = data.get("blocks", data) if isinstance(data, dict) else data
    if not isinstance(blocks_out, list):
        blocks_out = []

    # Map to BlockEditResult with feedback_edit for merge_node (development + content or line + copy)
    feedback_keys = ("development", "content") if combined_editor_type == "development+content" else ("line", "copy")
    block_results = []
    for blk in blocks_out:
        if not isinstance(blk, dict):
            continue
        bid = blk.get("id", "")
        original_text = blk.get("original_text", "")
        final_text = blk.get("final_text", original_text)
        editorial = blk.get("editorial_feedback") or {}
        items = []
        for k in feedback_keys:
            items.extend(editorial.get(k) or [])
        if not items and feedback_keys[0] in editorial:
            items = editorial.get(feedback_keys[0]) or []
        if not items and feedback_keys[1] in editorial:
            items = editorial.get(feedback_keys[1]) or []
        feedback_items = [FeedbackItem(issue=i.get("issue", ""), fix=i.get("fix", ""), impact=i.get("impact", ""), rule_used=i.get("rule_used", ""), priority=(i.get("priority") or "Enhancement")) for i in items]
        # So merge_node can place feedback in both keys
        feedback_edit = [SingleEditorFeedback(editor=feedback_keys[0], items=feedback_items), SingleEditorFeedback(editor=feedback_keys[1], items=feedback_items)]
        block_results.append(BlockEditResult(
            id=bid,
            type=blk.get("type", "paragraph"),
            level=int(blk.get("level", 0)),
            original_text=original_text,
            suggested_text=final_text,
            has_changes=(final_text.strip() != original_text.strip()),
            feedback_edit=feedback_edit,
        ))
    return EditorResult(editor_type=combined_editor_type, blocks=block_results, warnings=[])


def combine_editors_node(state: SupervisorState, combined_editor_type: str) -> SupervisorState:
    """Replace last two editor_results with one combined result (from LLM merge)."""
    editor_results = state.get("editor_results", [])
    if len(editor_results) < 2:
        return state
    result_a, result_b = editor_results[-2], editor_results[-1]
    combined = _run_combine_editors(result_a, result_b, combined_editor_type, state["document"])
    return {
        **state,
        "editor_results": editor_results[:-2] + [combined],
    }


# ---------------------------------------------------------------------
# DEVELOPMENT + CONTENT COMBINED NODE (run both tools then combine)
# ---------------------------------------------------------------------
def development_content_combined_node(state: SupervisorState) -> SupervisorState:
    """Run development tool, then content tool, then combine node (same-sentence rule + impact)."""
    logger.info("RUNNING: development_content_combined_node")
    s = {**state}
    # 1. Article analysis
    out = article_analysis_node(s)
    s = {**s, **out}
    # 2. Development editor
    out = development_editor_node(s)
    s = {**s, **out}
    # 3. Validation loop (up to 2 retries)
    for _ in range(3):
        out = article_validation_node(s)
        s = {**s, **out}
        val = s.get("validation_result")
        if val and getattr(val, "score", 0) >= 8:
            break
        if _ < 2:
            out = development_editor_retry_node(s)
            s = {**s, **out}
    # 4. Cross-paragraph analysis
    out = cross_paragraph_analysis_node(s)
    s = {**s, **out}
    # 5. Content editor
    out = content_editor_node(s)
    s = {**s, **out}
    # 6. Content validation loop (up to 2 retries)
    for _ in range(3):
        out = content_validation_node(s)
        s = {**s, **out}
        val = s.get("content_validation_result")
        if val and getattr(val, "score", 0) >= 8:
            break
        if _ < 2:
            out = content_editor_retry_node(s)
            s = {**s, **out}
    # 7. Combine development + content into one result
    return combine_editors_node(s, "development+content")


# ---------------------------------------------------------------------
# LINE + COPY COMBINED NODE (run both tools then combine)
# ---------------------------------------------------------------------
def line_copy_combined_node(state: SupervisorState) -> SupervisorState:
    """Run line tool, then copy tool, then combine node (same-sentence rule + impact)."""
    logger.info("RUNNING: line_copy_combined_node")
    s = {**state}
    out = line_editor_node(s)
    s = {**s, **out}
    out = copy_editor_node(s)
    s = {**s, **out}
    return combine_editors_node(s, "line+copy")
...
def route_sequential_editor(state: SupervisorState):
    """Route to current editor based on current_editor_index. Three options: development+content, line+copy, brand-alignment."""
...
def build_sequential_graph():
    ...
    # Set entry point - three options: development+content, line+copy, brand-alignment
    ...
    graph.add_edge("brand_editor_tool", "merge")
