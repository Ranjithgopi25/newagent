...
"""
    ...
    # Brand Editor: inject operational RISK_WORDS_DICTIONARY so mechanical matching can run.
    # Previously this was cleared, so the model had no dictionary and risk phrases were never flagged.
    if editor_type in ("brand", "brand-alignment"):
        try:
            ...
            logger.warning(
                "Risk words CSV missing; brand editor running without dictionary. "
                "Place a CSV in edit_content/risk/ with columns Word/Phrase, etc."
            )
            prompt = prompt.replace("{risk_words_instruction}", "")
    else:
        prompt = prompt.replace("{risk_words_instruction}", "")
