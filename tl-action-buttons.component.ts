...
# ---------------------------------------------------------------------
# FORMAT AND MARKDOWN (single LLM call: formatting + markdown structure)
# ---------------------------------------------------------------------
def format_and_markdown_article_with_llm(final_article: str, block_types: Optional[List[dict]] = None) -> str:
    """
    Apply final formatting and markdown structure in one LLM call.
    Returns formatted article with both formatting fixes and markdown structure (# ## ###, ## References 1. 2. 3.) applied.
    """
    if not final_article or not final_article.strip():
        return final_article
    try:
        prompt = FINAL_FORMATTING_AND_MARKDOWN_PROMPT
        ...
        if text.startswith("```"):
            ...
            if lines and lines[-1].strip() == "```":
                lines = lines[:-1]
            text = "\n".join(lines).strip()
        return text if text else final_article
    except Exception as e:
        logger.error(f"Error in format_and_markdown_article_with_llm: {e}", exc_info=True)
        return final_article
...
async def generate_final_article(request: FinalArticleRequest):
    ...
    # Apply final formatting and markdown structure in one LLM call
    final_article = format_and_markdown_article_with_llm(final_article, block_types)
